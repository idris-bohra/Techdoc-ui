...
const tokenDataReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case tokenDataActionTypes.UPDATE_TOKEN:
      state.tokenDetails[action.payload.tokenId].refreshToken =
        action?.payload?.refreshToken || null;
      state.tokenDetails[action.payload.tokenId].expiryTime =
        action?.payload?.expiryTime || null;
      state.tokenDetails[action.payload.tokenId].accessToken =
