...
export const fetchAllCookies = () => {
  return (dispatch) => {
    cookiesApiService
      ...
      .catch((error) => {
        dispatch(
          onCookiesFetchedError(error.response ? error.response.data : error)
        );
      });
  };
};
...
export const addCookieDomain = (data) => {
  return (dispatch) => {
    ...
    cookiesApiService
      ...
      .catch((error) => {
        dispatch(
          onDomainAddedError(error.response ? error.response.data : error, data)
        );
      });
  };
};
...
export const updateCookies = (data) => {
  return (dispatch) => {
    ...
    const originalData = JSON.parse(
      JSON.stringify(store.getState().cookies[id])
    );
