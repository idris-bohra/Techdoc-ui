    ...
    return (
      <div className='form-group'>
        <form className='d-flex p-2' onSubmit={(e) => this.handleSubmit(e)}>
          ...
          />
          <button
            className='btn btn-primary btn-sm font-12 ml-3'
            type='submit'
            disabled={this.state.currentDomain.domain === ''}
          >
            Add
          </button>
        </form>
        {this.props.validityMessage && (
          <small className='muted-text'>*Please enter valid Domain</small>
        )}
      </div>
    );
  }
    ...
    return Object.keys(this.props.domains)?.length > 0 ? (
      Object.values(this.props.domains).map((domain, index) => (
        ...
        >
          ...
          <div
            className='cursor-pointer ml-2 icon-center'
      ...
      <h4 className='text-center font-12 text-secondary'>
        No Domain available!
      </h4>
    );
  }
