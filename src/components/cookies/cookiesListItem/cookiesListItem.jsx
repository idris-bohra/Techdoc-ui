...
class CookiesListItem extends Component {
  ...
  saveCookie() {
    ...
    if (currentDomain.cookies[currentCookie.key] === currentCookie.value) {
      ...
      const { cookieString, name, expiresValue } = this.prepareCookieString(
        currentCookie.value.trim()
      );
      ...
      <>
        ...
        <div className='text-right mt-2'>
          {this.showDisableButton() && (
            <span
              className='cursor-pointer mr-3 btn default'
              onClick={() => this.setState({ updateCookie: {} })}
            >
              Update
            </span>
          )}
          <button
            ...
            disabled={!this.showDisableButton()}
