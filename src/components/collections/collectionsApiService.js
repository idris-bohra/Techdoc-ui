...
export function duplicateCollection(collectionId) {
  return http.post(
    `${apiUrl}/orgs/${orgId}/duplicateCollections/${collectionId}`
  );
}

export function importCollectionService(
  importObject,
  uniqueTabId,
  defaultView,
  orgDetails
  ...
  const orgId = getOrgId();
  return http.post(
    `${apiUrl}/orgs/${orgId}/importCollection?defaultView=${defaultView}&uniqueTabId=${uniqueTabId}&orgName=${orgDetails.orgName}`,
    importObject
  );
}
