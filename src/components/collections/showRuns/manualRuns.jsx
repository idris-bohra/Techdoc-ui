...
function ManualRuns() {
  const { automation, activeTabId, collections, pages } = useSelector(
    (state) => {
      return {
  ...
  const averageResponseTime = useMemo(() => {
    ...
    const executionOrderLength =
      automation[activeTabId]?.executionOrder?.length || 1;
  ...
  return (
    <div className='manual-runs-container'>
      ...
      <div className='run-details'>
        ...
        <a
          className='ml-2'
          href={`/orgs/${orgId}/dashboard/collection/${collectionId}/runs`}
        >
          View all runs
        </a>
      </div>
      ...
      <div className='test-results'>
        ...
        <div className='iteration-details'>
          {automation[activeTabId]?.executionOrder.map((id) => (
            <>
              <div
                className='iteration-details-api d-flex justify-content-between'
                key={id}
              >
                <div>
                  <span
                    className={`${pages[id]?.requestType} request-type-bgcolor mr-2`}
                  >
                    {pages[id]?.requestType}{' '}
                  </span>
                ...
                <div className='run-details'>
                  <span>
                    {
                      automation[activeTabId]?.executedScriptResponses[id]
                        ?.requestDuration
                    }
                    ms
                  </span>
              ...
              <div>
                {Object.entries(
                  automation[activeTabId]?.executedScriptResponses[id]?.data ||
                    {}
                ).map(([key, value]) => (
