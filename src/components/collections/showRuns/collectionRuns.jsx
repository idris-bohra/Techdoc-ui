...
const CollectionRuns = () => {
  ...
  const { scheduledRuns, deleteCronById, updateCronStatus } =
    ScheduledRunsActions(params?.collectionId, activeTab);
  ...
  const openManualLogs = async (runId) => {
    ...
    router.push(
      `/orgs/${orgId}/dashboard/collection/${collectionId}/runs/${runId}`
    );
  };
  ...
  const openEditCron = async (cronId) => {
    const collectionId = params?.collectionId;
    router.push(
      `/orgs/${orgId}/dashboard/collection/${collectionId}/cron/${cronId}/edit`
    );
  };
      ...
      <div className='content'>
          ...
          <table>
            ...
            <tbody>
              {Object.entries(automation)
                .filter(
                  ([id, runDetails]) =>
                    runDetails.collectionId === params.collectionId
                )
                .map(([id, runDetails]) => (
                  <tr key={id}>
                    <td>
                      {moment(runDetails.date).format(
              ...
              {scheduledRuns.map((run) => (
                <tr key={run.id}>
                  <td>
                    {' '}
                    {run.status === 1 ? (
                      cronstrue.toString(run.cron_expression)
                    ) : (
                  ...
                  <div className='position-relative'>
                    ...
                    <div className='dropdown-menu dropdown-menu-right'>
                      <div
                        ...
                        onClick={() =>
                          updateCronStatus(run?.id, run?.status === 1 ? 0 : 1)
                        }
                      >
                        {run?.status === 1 ? (
                          <MdOutlineMotionPhotosPaused />
                        ...
                        <span className='ml-2'>
                          {run?.status === 1 ? 'Pause' : 'Resume'}
                        </span>
                      </div>
                      <div
                        className='dropdown-item d-flex align-items-center'
                        onClick={() => openEditCron(run?.id)}
                      >
                        <MdModeEdit />
        ...
        {activeTab === 'scheduled' && scheduledRuns?.length === 0 && (
          <div className='text-center mt-5'>
            ...
            <p>
              You can schedule runs for this collection to periodically run it
              at a certain time or frequency on the Techdoc Cloud.
            </p>
            <Button
              className='btn btn-primary fs-sm mr-1'
              onClick={navigateToScheduleRuns}
            >
              Schedule Runs
            </Button>
          </div>
