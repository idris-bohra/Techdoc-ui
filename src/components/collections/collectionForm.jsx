...
const CollectionForm = (props) => {
  ...
  const redirectToCollection = (collection) => {
    ...
    if (collection.success)
      router.push(
        `/orgs/${params.orgId}/dashboard/collection/${collection.data.id}/settings`
      );
    props.onHide();
  };
  ...
  const doSubmit = async () => {
    ...
    const collection = await dispatch(
      addCollection(collectionData, null, redirectToCollection)
    );
  ...
  return (
    ...
    >
      ...
      <Modal.Body>
        <Input
          defaultValue={collections[props?.collectionId]?.name}
          name='name'
          urlName='Name'
          label='Collection Name'
