...
export const addCollection = (newCollection, customCallback) => {
  newCollection.uniqueTabId = sessionStorage.getItem(
    SESSION_STORAGE_KEY.UNIQUE_TAB_ID
  );
  return (dispatch) => {
    return new Promise((resolve, reject) => {
      collectionsApiService
        ...
        .catch((error) => {
          ...
          toast.error(
            error.response ? error.response.data : 'Error adding collection'
          );
          reject(error);
        });
    });
...
export const updateCollection = (
  ...
  return (dispatch) => {
    editedCollection.uniqueTabId = sessionStorage.getItem(
      SESSION_STORAGE_KEY.UNIQUE_TAB_ID
    );
    const originalCollection =
      store.getState().collections[editedCollection.id];
    dispatch(
      updateCollectionRequest({ ...originalCollection, ...editedCollection })
    );
...
export const deleteCollection = (collection) => {
  collection.uniqueTabId = sessionStorage.getItem(
    SESSION_STORAGE_KEY.UNIQUE_TAB_ID
  );
  return (dispatch) => {
    collectionsApiService
      ...
      .then((res) => {
        ...
        deleteAllPagesAndTabsAndReactQueryData(rootParentPageId, collection.id)
          ...
          .catch((error) => {
            console.error(
              'error after getting data from deleteCollection deleteAllPagesAndTabsAndReactQueryData == ',
              error
            );
          });
      })
...
export const importCollection = (
  ...
  return async (dispatch) => {
    try {
      ...
      const response = await collectionsApiService.importCollectionService(
        collection,
        uniqueTabId,
        defaultView,
        orgDetails
