...
function PublishSidebar(props) {
  ...
  const [flattenData, setFlattenData] = useState([
    { name: '', id: 0, children: [], parent: null },
  ]);
  ...
  const getModifiedData = () => {
    const data1 = modifyDataForBulkPublish(
      collections,
      pages,
      params.collectionId
    );
  ...
  const sendPublishRequest = () => {
    ...
    if (allSelectedIds?.length === 0)
      return toast.error('Please Select Something To Publish');
    ...
    const pageIds = Array.from(dataToPublish).map(
      (id) => flattenData?.[id]?.metadata?.actualId
    );
    try {
  ...
  const ArrowIcon = ({ isOpen }) => {
    return !isOpen ? (
      <MdOutlineArrowDropDown color='black' size={22} />
    ) : (
      <MdArrowDropUp color='black' size={22} />
    );
  };
  ...
  const CheckBoxIcon = ({ variant, ...rest }) => {
    switch (variant) {
      ...
      case 'all':
        return (
          <IoIosCheckbox {...rest} color='var(--checkbox-color-primary)' />
        );
      case 'some':
    ...
    return (
      <div className='d-flex'>
        ...
        <Button
          variant='btn btn-outline'
          className='m-1 btn-sm font-12'
          onClick={() => props.closePublishSidebar()}
        >
          Cancel
        </Button>
      </div>
        ...
        <div className='checkbox mt-3'>
          <TreeView
            ...
            nodeRenderer={({
              ...
              const requestType = element.metadata?.actualId
                ? pages?.[element.metadata?.actualId]?.requestType
                : null;
              return (
                <div
                ...
                >
                  ...
                  <span className='name element-name'>
                    ...
                    {pages?.[element.metadata?.actualId]?.type === 5 ? (
                      ...
                      <>
                        {requestType &&
                          pages?.[element.metadata?.actualId]?.protocolType ===
                            1 && (
                            <div
                              className={`api-label lg-label ml-2 ${requestType}`}
                            >
