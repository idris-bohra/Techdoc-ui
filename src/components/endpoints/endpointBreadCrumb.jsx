...
class EndpointBreadCrumb extends Component {
  ...
  handleInputBlur(event) {
    ...
    if (
      newName !== this.state.previousTitle &&
  ...
  setEndpointData() {
    ...
    this.collectionName = this.collectionId
      ? this.props.collections[this.collectionId]?.name
      : null;
  }
  ...
  handleProtocolTypeClick(index) {
    ...
    this.props.update_tab(
      this.props?.params.endpointId === 'new' && this.props.activeTabId,
      { isModified: true }
    );
  ...
  switchProtocolTypeDropdown() {
    return (
      <div className='dropdown d-flex justify-content-center align-items-center'>
        <button
          className='protocol-selected-type d-flex justify-content-center align-items-center'
        ...
        >
          {
            this.state.protocols[this.props?.endpointContent?.protocolType - 1]
              ?.icon
          }
        </button>
        <div
          className='dropdown-menu protocol-dropdown'
        ...
        >
          {this.state.protocols.map((protocolDetails, index) => (
            <button
              ...
              key={index}
              onClick={() => this.handleProtocolTypeClick(index)}
            >
  ...
  renderPathLinks() {
    ...
    const pathWithUrls = this.getPath(
      this.props?.params?.pageId || this.props?.params?.endpointId,
      this.props.pages
    );
    return pathWithUrls.map((item, index) => {
      ...
      return (
        <div
          ...
          onClick={() => this.handleOnPathVarClick(isLastItem, item)}
        >
          {isLastItem ? (
            <strong
              contentEditable
            ...
            >
              {this.props?.isEndpoint
                ? this.props?.pages?.[this.props?.params?.endpointId]?.name ||
                  this.props?.history?.[this.props?.params?.historyId]?.endpoint
                    ?.name ||
                  this.props?.endpointContent?.data?.name
                : this.props?.pages?.[this.props?.params?.pageId]?.name}
            </strong>
          ) : (
            <strong className='cursor-pointer fw-400 cursor-pointer px-1 py-0 text-secondary fw-400'>
              {item.name}
            </strong>
          )}
      ...
      <div className='endpoint-header d-flex align-items-center'>
        <div className='panel-endpoint-name-container d-flex align-items-center'>
          <div className='page-title-name d-flex align-items-center'>
            {this.props?.tabState[this.props?.activeTabId]?.status === 'NEW' &&
              this.switchProtocolTypeDropdown()}
          </div>
          ...
          {this.props.tabState[this.props.activeTabId]?.status !== 'NEW' ? (
            <div className='d-flex bread-crumb-wrapper align-items-center text-nowrap'>
              ...
              {this.props.publishLoader && (
                <div>
                  <div
                    ...
                    role='status'
                    style={{ color: '#6c757d ', width: '1rem', height: '1rem' }}
                  >
              ...
              {this.props?.endpoints[this.props.currentEndpointId]
                ?.isPublished && (
                <OverlayTrigger
                  placement='right'
                  overlay={
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(EndpointBreadCrumb)
);
