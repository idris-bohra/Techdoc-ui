...
class GenericTable extends Component {
  ...
  handleChange = (e, inpTarget = null) => {
    ...
    if (
      ...
      dataArray[name[0]].checked === 'notApplicable' &&
  ...
  handleBulkChange = (e) => {
    ...
    if (title === 'formData' || title === 'x-www-form-urlencoded') {
      this.props.handle_change_body_data(title, [
        ...dataArrayOfFileType,
        ...obj,
      ]);
    }
    ...
    if (count) {
      if (
        ...
        for (let index = 0; index < dataArray?.length; index++) {
          ...
          if (checked === 'true') {
            textAreaValue +=
              getInnerText(dataArray[index].key) +
              ':' +
              getInnerText(dataArray[index].value) +
              '\n';
          } else {
            textAreaValue +=
              '//' +
  ...
  renderPublicTableRow(dataArray, index, originalData, title) {
    ...
    return (
      <tr
        key={currentItem.key}
        id='generic-table-row'
        className={
          getHighlightsData(this.props, title, [currentItem.key])
            ? 'active'
            : ''
        }
      >
        <td
          className='custom-td custom-checkbox-public'
        ...
        <td className='custom-td valueWrapper'>
          <div className='d-flex align-items-center'>
            <GenericTableAutoSuggest
              ...
              htmlValue={dataArray[index].value}
            />
            {isEmpty && (
  ...
  handleSelect = (selectedOption, name) => {
    ...
    if (
      this.props.title === 'Headers' ||
      this.props.title === 'Params' ||
      this.props.title === 'Path Variables'
    ) {
      this.props.props_from_parent(this.props.title, dataArray);
    }
    if (
      this.props.title === 'formData' ||
      this.props.title === 'x-www-form-urlencoded'
    ) {
    ...
    return (
      <div className='position-relative fileInput'>
        <GenericTableAutoSuggest
          ...
          valueKey={key}
          handleChange={this.handleChange}
          htmlValue={dataArray[index].key}
          disable={
            title === 'Path Variables' && key.split('.')[1] === 'key'
              ? true
              : false
          }
        />
    ...
    return (
      <tr key={index} id='generic-table-row'>
            ...
            <label className='customCheckbox'>
              <input
                disabled={
                  ...
                  originalData[index].type != 'enable'
                    ? null
                    : 'disabled'
                }
                name={index + '.checkbox'}
        ...
        <td
          className='custom-td'
          style={{ width: '435px' }}
          id='generic-table-description-cell'
        >
          {isDashboardRoute(this.props) ? (
            <div>
              <input
        ...
        {isDashboardRoute(this.props) ? (
          <div className='delete-icons'>
            ...
            title === 'Path Variables' ? null : (
              <button
                type='button'
                className='btn pl-2 pt-1 pr-0 cross-button'
                onClick={() => this.handleDelete(dataArray, index, title)}
              >
                <MdDeleteOutline size={20} className='text-link' />
              </button>
            )}
          </div>
        ) : (
  ...
  renderSelectFiles(dataArray, index) {
    ...
    return (
      <div className='selectFile d-flex align-items-center'>
        <button onClick={() => this.handleFileInput(dataArray, index)}>
          Select file
        </button>
      </div>
    );
    ...
    return !isDashboardRoute(this.props) && this.findUncheckedEntityCount() ? (
      <IconButton>
        <div
          ...
          onClick={() => this.toggleOptionalParams()}
  ...
  render() {
    ...
    return (
      <div className='hm-public-table position-relative mb-2'>
        {isOnPublishedPage() && (
          <div className='public-generic-table-title-container mt-2'>
            {getTitleHeading()}
          </div>
        )}
        {title === 'Path Variables' &&
          ...
          this.props?.dataArray?.length > 0 && (
            <div className='font-12 fw-500 my-1 text-secondary'>{title}</div>
          )}
        {title === 'Params' &&
          isDashboardAndTestingView(this.props, this.props.currentView) &&
          this.props?.dataArray?.length > 0 && (
            <div className='font-12 fw-500 my-1 text-secondary'>
              Query Params
            </div>
          )}
        {!this.state.bulkEdit && dataArray?.length > 0 ? (
          <div
            className={`headParaWraper mb-1 p-0`}
              ...
              {isDashboardRoute(this.props) ? (
                <thead>
                  <tr>
                    ...
                    <th
                      className='custom-th text-grey'
                      id='generic-table-key-cell dd'
                    >
                      KEY
                    </th>
        ...
        {title === 'Path Variables' || !isDashboardRoute(this.props) ? null : (
          <div className='generic-table-title-container'>
            <button
              className={`adddescLink mt-2 ${title === 'Params' ? 'addBulk-params' : 'addBulk'} icon-button px-2`}
