...
export default function QueryTab(props) {
  ...
  const { activeTabId, introspectionSchemaData, currentEnvironment } =
    useSelector((state) => {
      return {
        activeTabId: state.tabs.activeTabId,
        introspectionSchemaData:
          state?.tabs?.tabs?.[state.tabs.activeTabId]?.introspectionSchemaData,
        currentEnvironment:
  ...
  const callIntrospectionQueryAPI = async () => {
    const graphQlUrl =
      props.replaceVariables(
        getInnerText(props?.endpointContent?.data.URL),
    ...
    if (graphQlUrl) {
      try {
        ...
        const responseData =
          await getSchemaThroughIntrospectionQuery(graphQlUrl);
      ...
      <React.Fragment>
        {loadingStateForSchema ? (
          <div className='loading-schema'>
            {[1, 2, 3, 4, 5, 6]?.map((item) => {
              return (
                <div
                  key={item}
                  className='d-flex schema-box-container align-items-center gap-3 mt-2'
                >
          ...
          <div className='query-schema d-flex flex-column align-items-center justify-content-center'>
            ...
            <span className='text-grey'>
              Enter server URL to load schema using introspection and refresh
              it.
            </span>
          </div>
        )}
        ...
        <div className='d-flex justify-content-between align-items-center query-schema-head ml-1'>
          ...
          <IconButton>
            <MdOutlineRefresh
              className='text-grey'
              size={18}
              onClick={callIntrospectionQueryAPI}
            />
          </IconButton>
        </div>
        {introspectionSchemaData && !loadingStateForSchema ? (
          <ShowIndentedSchema loadedSchema={introspectionSchemaData} />
        ) : (
          <ShowSchemaMessage />
        )}
      </div>
