    ...
    <div className='accordion-container'>
      {parents.map((parentName, index) => {
        return (
          <div key={index}>
            <div
              className='d-flex align-items-center parent-accordion-container'
              onClick={() => handleArrowClick(parentName)}
            >
              ...
              <span className='ml-1'>
                {parentName === 'queryType' ? 'Query' : 'Mutation'}
              </span>
            </div>
            {queryMutationState?.[parentName] && (
              <div className='mt-2 mb-2'>
                {props?.loadedSchema?.[parentName]?.fields?.map(
                  (fieldData, index) => {
