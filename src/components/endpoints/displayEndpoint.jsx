...
import {
  closeTab,
  updatePostPreScriptExecutedData,
  updateTab,
  updateTabDraft,
...
import {
  addAuthorizationDataTypes,
  grantTypesEnums,
} from '../common/authorizationEnums.js';
...
import {
  ...
  getPathVariableHTML,
  getQueryParamsHTML,
} from '../../utilities/htmlConverter.js';
...
const mapStateToProps = (state) => {
  return {
    ...
    currentEnvironment:
      state?.environment?.environments[state?.environment?.currentEnvironmentId]
        ?.variables || {},
...
const mapDispatchToProps = (dispatch, ownProps) => {
  return {
    ...
    example_request: (id, editedEndpoint, callback) =>
      dispatch(
        addExampleRequest(ownProps.navigate, id, editedEndpoint, callback)
      ),
    ...
    approve_endpoint: (endpoint, callback) =>
      dispatch(approveEndpoint(endpoint, callback)),
    ...
    update_pre_post_script: (tabId, executionData) =>
      dispatch(updatePostPreScriptExecutedData(tabId, executionData)),
...
const getEndpointContent = async (props, currentPublicId) => {
  let isUserOnPublishedPage = isOnPublishedPage();
  let currentIdToShow = isUserOnPublishedPage
    ? currentPublicId
    : null;
  ...
  const extractedParams = extractParams(
    '/orgs/:orgId/dashboard/endpoint/:endpointId',
    window.location.pathname
  );

  if (
    extractedParams?.endpointId !== 'new' &&
    props?.pages?.[endpointId] &&
    endpointId
  ) {
    let type = props?.pages?.[currentIdToShow]?.type;
    let data = isUserOnPublishedPage
      ? await getPublishedContentByIdAndType(currentIdToShow, type)
      : await getEndpoint(endpointId);
    return utilityFunctions.modifyEndpointContent(
      data,
      _.cloneDeep(untitledEndpointData)
    );
  }
...
const fetchHistory = (historyId, props) => {
  ...
  return {
    ...utilityFunctions.modifyEndpointContent(
      _.cloneDeep(data),
      _.cloneDeep(untitledEndpointData)
    ),
    flagResponse: true,
...
const withQuery = (WrappedComponent) => {
  return (props) => {
    ...
    let currentIdToShow = isOnPublishedPage()
      ? currentPublicId
      : null;
    ...
    if (historyId && historyId !== 'new') {
      ...
      fetchFunction = async () => {
        return getEndpointContent(props, currentPublicId);
      };
    }
    ...
    const setOldDataToNewDataForBody = (data) => {
      ...
      if (data?.protocolType != 1) {
        ...
        bodyType === bodyTypesEnums['application/x-www-form-urlencoded'] ||
        bodyType === bodyTypesEnums['multipart/form-data']
      ) {
    ...
    const setQueryUpdatedData = (data, callbackFn = null) => {
      let currentIdToShow = currentPublicId
      const endpointId =
        params.endpointId !== 'new'
          ? params?.endpointId || currentIdToShow
          : props?.activeTabId;
...
class DisplayEndpoint extends Component {
  constructor(props) {
    super(props);
    this.handleRemovePublicEndpoint =
      this.handleRemovePublicEndpoint.bind(this);
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      ...
      this.props?.tabs[this.props?.activeTabId]?.type === 'endpoint'
    ) {
    ...
    if (
      ...
      (!_.isEqual(
        this.state?.endpointContentState?.data,
        this.props?.endpointContent?.data
      ) ||
        !_.isEqual(
          this.state?.endpointContentState?.originalParams,
          this.props?.endpointContent?.originalParams
        ) ||
        !_.isEqual(
          this.state?.endpointContentState?.originalHeaders,
          this.props?.endpointContent?.originalHeaders
        ) ||
        !_.isEqual(
          this.state?.endpointContentState?.pathVariables,
          this.props?.endpointContent?.pathVariables
        ) ||
        !_.isEqual(
          this.state?.endpointContentState?.host,
          this.props?.endpointContent?.host
        ))
    ) {
    ...
    if (
      this.props?.endpointContent &&
      !_.isEqual(this.props.endpointContent, this.state.endpointContentState)
    ) {
      this.setState({
        endpointContentState: _.cloneDeep(this.props.endpointContent),
      });
  ...
  handleKeyDown = (event) => {
    ...
    if (
      (isMac && event.metaKey && event.key === 'm') ||
      (!isMac &&
        event.ctrlKey &&
        event.key === 'm' &&
    ...
    if (
      (isMac && event.metaKey && event.key === 'q') ||
      (!isMac &&
        event.ctrlKey &&
        event.key === 'q' &&
        this.props?.tabs[this.props?.activeTabId]?.status !== 'NEW' &&
        this.props.pages[this.endpointId]?.isPublished)
    ) {
      event.preventDefault();
  ...
  handleChange = (e) => {
    ...
    if (e.currentTarget.name === 'URL') {
      ...
      originalParams = makeOriginalParams(
        keys,
        values,
        description,
  ...
  setPathVariables(value) {
    ...
    for (let i = 0; i < pathVariablesHtmlData?.length; i++) {
      pathVariables.push({
        ...
        description:
          pathVariableData[getInnerText(pathVariablesHtmlData[i])]?.description,
      });
    }
  ...
  async handleApiCall(
    ...
    const currentEndpointId =
      this.props.currentEndpointId !== 'new'
        ? this.props.activeTabId
        : this.props.currentEndpointId;
    let responseJson = {};
    try {
  ...
  processResponse(responseJson) {
    ...
    if (responseJson?.data?.success === false) {
      ...
      response = { status, statusText, data: error };
  ...
  setPathVariableValues(uris, env) {
    ...
    this.props.endpointContent.pathVariables.forEach((variable) => {
      pathVariablesMap[
        replaceVariables(getInnerText(variable.key), env.variables)
      ] = replaceVariables(getInnerText(variable.value), env.variables);
    });
    ...
    for (let i = 0; i < pathParameters?.length; i++) {
      if (pathParameters[i][0] === ':') {
        ...
        if (
          pathVariablesMap.hasOwnProperty(pathVariableKey) &&
          pathVariablesMap[pathVariableKey] !== ''
        ) {
  ...
  setData = async () => {
    ...
    if (
      this.props?.endpointContent?.data?.body?.type === bodyTypesEnums['raw']
    ) {
    ...
    const endpoint = {
      ...
      bodyDescription:
        this.props.endpointContent.data.body.type === rawTypesEnums.JSON
          ? this.props.endpointContent.bodyDescription
          : {},
  ...
  checkEmptyParams() {
    ...
    params.forEach((param) => {
      if (
        param.checked !== 'notApplicable' &&
        param.checked === 'true' &&
        this.checkValue(param, originalParams)
      ) {
        isEmpty = true;
        param.empty = true;
      } else {
  ...
  addhttps(url) {
    if (url) {
      if (
        this.props?.endpointContent?.data?.updatedUri?.includes('localhost') &&
  ...
  async getRefreshToken(headers, url) {
    let oauth2Data =
      this.props?.endpointContent?.authorizationData?.authorization?.oauth2;
    if (
      this.props?.endpointContent?.authorizationData
        ?.authorizationTypeSelected === 'oauth2' &&
      (this.props.tokenDetails?.[oauth2Data?.selectedTokenId]?.grantType ===
        grantTypesEnums.authorizationCode ||
        this.props.tokenDetails?.[oauth2Data?.selectedTokenId]?.grantType ===
        grantTypesEnums.authorizationCodeWithPkce)
    ) {
      const generatedDateTime =
        this.props.tokenDetails?.[oauth2Data?.selectedTokenId]?.createdTime;
      const expirationTime =
        this.props.tokenDetails?.[oauth2Data?.selectedTokenId]?.expiryTime;
      const isTokenExpired = this.checkTokenExpired(
        expirationTime,
        generatedDateTime
      );
      if (
        ...
        try {
          const data = await endpointApiService.getRefreshToken(
            this.props.tokenDetails[oauth2Data.selectedTokenId]
          );
          if (data?.access_token) {
            ...
            if (
              oauth2Data?.addAuthorizationRequestTo ===
    ...
    matches.forEach((match) => {
      ...
      if (suggestion) {
        const valueToReplace =
          suggestion.currentValue || suggestion.initialValue;
  ...
  handleSend = async () => {
    ...
    Object.keys(headersData).forEach((header) => {
      headerJson[getInnerText(header)] = getInnerText(
        headersData[header].value
      );
    });
    ...
    const path = this.setPathVariableValues(
      ...
      !isOnPublishedPage()
        ...
        : { variables: this.props?.publicEnv }
    );
    ...
    const method = this.checkProtocolType(1)
    ...
    if (!isOnPublishedPage()) {
      ...
      url = replaceVariables(url || {}, this.props?.publicEnv);
      url = this.addhttps(url);
      headers = replaceVariablesInJson(headers || {}, this.props?.publicEnv);
      const bodyType = this.props?.endpointContent?.data?.body?.type;
      body = replaceVariablesInBody(body, bodyType, this.props?.publicEnv);
      requestOptions = { ...requestOptions, body, headers, url, bodyType };
    }
  ...
  handleSave = async (id, endpointObject, slug, saveType = null) => {
    ...
    if (
      (currentTabId &&
        !this.props.pages[currentTabId] &&
        !this.state.showEndpointFormModal) ||
      (this.props?.params?.historyId && slug !== 'isHistory')
    ) {
      this.openEndpointFormModal();
    } else {
      let endpointContent = this.props.getDataFromReactQuery([
        'endpoint',
        currentTabId,
      ]);
      ...
      updatedPathVariables = Object.keys(updatedPathVariables).reduce(
        (obj, key) => {
      ...
      let endpoint = {
        ...
        name: this.state.saveAsFlag
          ? endpointName
          : endpointContent?.data?.name,
  ...
  propsFromChild(name, value, targetName, index) {
    if (name === 'Params') {
      ...
      this.props.setQueryUpdatedData(
        dummyData,
        this.prepareHarObject.bind(this)
      );
    }
  ...
  setPublicBody(body) {
    const data = { ...this.props?.endpointContent?.data };
    data.body = {
      type: data?.body?.type ?? bodyTypesEnums['none'],
      value: body,
    };
  ...
  handleUpdateUri(originalParams) {
    ...
    if (
      queryParamsHtmlData?.length != 0 &&
      queryParamsHtmlData[0]?.key?.startIndex
    ) {
    ...
    originalParams.forEach((params) => {
      if (params.checked == 'true') {
        ...
        paramsHTML =
          paramsHTML + `${params.key}${getIntoTextBlock('=')}${params.value}`;
        count++;
      }
    });
    ...
    if (count === 0) {
      if (counter === -1 || counter === 0) {
        ...
        if (
          newHTML.endsWith("<span text-block='true'>") ||
          newHTML.endsWith('<span text-block="true">')
        ) {
  ...
  async makePostData(body) {
    ...
    let postData = {};
    if (
      body.type === bodyTypesEnums['application/x-www-form-urlencoded'] ||
      body.type === bodyTypesEnums['multipart/form-data']
    ) {
      paramsFlag = true;
  ...
  async prepareHarObject() {
    ...
    const harObject = {
      ...
      postData:
        body && body.type !== bodyTypesEnums['none']
          ? await this.makePostData(body)
          : null,
  ...
  setHeaders(value, title) {
    ...
    if (title === 'oauth2' || title === 'basicAuth') {
      const authorizationKeyIndex =
        findAuthorizationKeyOrContentTypeIndex('Authorization');
      ...
      if (
        Object.keys(rawBodyTypes).includes(value) &&
        contentTypeKeyIndex > -1
      ) {
        originalHeaders[contentTypeKeyIndex].value = getIntoTextBlock(
          rawBodyTypes[value]
        );
      } else if (value === 'none') {
  ...
  deleteHeader() {
    ...
    for (let i = 0; i < originalHeaders?.length; i++) {
      if (
        ...
        originalHeaders[i].type === 'disable'
      ) {
        continue;
      } else {
  ...
  deleteParams() {
    ...
    for (let i = 0; i < originalParams?.length; i++) {
      if (
        originalParams[i].key === 'access_token' &&
        originalParams[i].type === 'disable'
      ) {
        continue;
      } else {
        updatedParams.push(originalParams[i]);
  ...
  addSampleResponse(response) {
    ...
    const sampleResponseArray = [
      ...this.props.endpointContent.sampleResponseArray,
      sampleResponse,
    ];
  ...
  displayResponse() {
    if (
      ...
      this.props?.endpointContent?.flagResponse
    ) {
      return (
        <div
          ref={this.myRef}
          className='hm-panel endpoint-public-response-container public-doc'
        >
          <DisplayResponse
            {...this.props}
      ...
      <>
        <div className='custom-tabs clear-both response-container mb-2 w-100'>
          <div className='d-flex justify-content-between align-items-center w-100'>
            <ul
              className='nav nav-tabs respTabsListing w-100 rounded-0 border-0'
              id='myTab'
              role='tablist'
            >
          ...
          <div className='tab-content responseTabWrapper' id='pills-tabContent'>
            ...
            >
              <div
                ...
                className='hm-panel endpoint-public-response-container '
              >
                <DisplayResponse
                  {...this.props}
                  loader={this.state?.loader}
      ...
      <>
        <div className='response-container endpoint-public-response-container endPointRes'>
          ...
          <DisplayResponse
            ...
            timeElapsed={this.state.timeElapsed}
            response={
              this.props?.historySnapshots[historyId]?.response || testResponse
            }
            flagResponse={this.props?.endpointContent?.flagResponse}
            handleCancel={() => {
              this.handleCancel();
  ...
  renderDocView = () => {
    ...
    if (isDashboardRoute(this.props)) {
      ...
      return this.props?.endpointContent?.docViewData?.map((item, index) => (
        <div key={index}>{this.renderPublicItem(item, index)}</div>
      ));
    }
  };
  ...
  renderDragHandle(item) {
    if (item.type === 'pathVariables') {
      if (
        this.props?.endpointContent?.pathVariables &&
        this.props?.endpointContent?.pathVariables?.length
      )
        return <DragHandle />;
      return;
    }
  ...
  onSortEnd = (oldIndex, newIndex) => {
    ...
    if (newIndex !== oldIndex) {
      ...
      this.props.setQueryUpdatedData(
        { ...this.props.endpointContent, docViewData: newData },
        () => this.setModifiedTabData()
      );
    }
  };
  ...
  renderPublicItem = (item, index) => {
    switch (item.type) {
      case 'textArea': {
        if (
          isDashboardRoute(this.props) ||
          (!isDashboardRoute(this.props) && item.data)
        ) {
      ...
      case 'textBlock': {
        if (
          ...
          return (
            <div
              className='pub-notes'
              style={{ borderLeftColor: this.state.theme }}
            >
      ...
      case 'host': {
        if (!isDashboardRoute(this.props)) return this.renderPublicHost();
          ...
          return (
            <div className='endpoint-url-container'> {this.renderHost()} </div>
          );
      }
      case 'body': {
        if (!isDashboardRoute(this.props))
          return this.renderPublicBodyContainer();
      ...
      case 'params': {
        if (!isDashboardRoute(this.props) && this.checkProtocolType(1))
          return this.renderPublicParams();
        else if (isDashboardRoute(this.props) && this.checkProtocolType(1))
          return <div className='mb-3'>{this.renderParams()}</div>;
        return null;
      }
      case 'pathVariables': {
        if (!isDashboardRoute(this.props))
          return this.renderPublicPathVariables();
        else return this.renderPathVariables();
      }
      case 'sampleResponse': {
        if (!isDashboardRoute(this.props))
          return this.displayPublicSampleResponse();
  ...
  isNotDashboardOrDocView() {
    return (
      !isDashboardRoute(this.props) ||
      this.props?.endpointContent?.currentView === 'doc'
    );
  }
  ...
  isDashboardAndTestingView() {
    return (
      isDashboardRoute(this.props) &&
      (this.props?.endpointContent?.currentView === 'testing' ||
          ...
          <Dropdown>
            ...
            >
              <FaPlus
                ...
                size={14}
                onClick={() => this.showDocOptions()}
              />
            </Dropdown.Toggle>
            <Dropdown.Menu
              id='bg-nested-dropdown'
              className='d-flex doc-plus-menu'
            >
              <Dropdown.Item onClick={() => this.addBlock('textArea')}>
                Text Area
              </Dropdown.Item>
              <Dropdown.Item onClick={() => this.addBlock('textBlock')}>
                Text Block
              </Dropdown.Item>
            </Dropdown.Menu>
          </Dropdown>
        ...
        <div className='accordion' id='accordionExample'>
          <div className='card'>
            <div className='card-header p-0' id='headingOne'>
              <h2
                className='mb-0 font-14 d-flex justify-content-between align-items-center'
                style={this.state.themes.backgroundStyles}
              ...
              >
                ...
                <IoIosArrowUp
                  size={18}
                  className='down-arrow mr-2 text-center'
                />
              </h2>
            </div>
            ...
            >
              <div className='card-body p-0'>
                <table
                  className='table border rounded py-3 my-0 pl-5 pr-3 pb-4'
                  style={this.state.themes.backgroundStyles}
                >
                  <thead>
                    <tr>
                      <th className='p-1 font-weight-normal font-12'>KEY</th>
  ...
  renderPublicHost() {
    return (
      <div>
          ...
          <div className='d-flex'>
            {this.checkProtocolType(1) && (
              <div className='input-group-prepend'>
                <span
                  className={`api-label api-label-lg input-group-text m-0 p-0 ${this.props?.endpointContent?.data?.method}`}
                >
                  {this.props?.endpointContent?.data?.method}
                </span>
              </div>
            ...
            <HostContainer
              ...
              updatedUri={_.cloneDeep(
                this.props?.endpointContent?.data?.updatedUri
              )}
              ...
              untitledEndpointData={_.cloneDeep(
                this.props.untitledEndpointData
              )}
            />
          </div>
        ...
        />
        {!isDashboardRoute(this.props) &&
          Object.keys(this.props.publicEnv)?.length !== 0 &&
          this.renderPublicEnvironments()}
      </div>
    );
  }
  ...
  renderHost() {
    ...
    return (
      <div
        className={`input-group-prepend ${this.props?.endpointContent?.currentView === 'doc' ? 'w-100' : ''}`}
      >
        {this.checkProtocolType(1) && (
          <div className='dropdown'>
            ...
            <div
              className='dropdown-menu dropdown-url'
        ...
        <div
          className={`d-flex dashboard-url ${this.state.addUrlClass ? 'Url-invalid' : ''} ${this.checkProtocolType(2) ? 'w-100' : ''}`}
        >
          <HostContainer
            {...this.props}
  ...
  renderSwitchBtn() {
    return (
      <div
        ...
        className='p-0 d-flex justify-content-between align-items-center cursor-pointer'
      >
    ...
    return (
      this.state.openUnPublishConfirmationModal && (
        <ConfirmationModal
          ...
          onHide={() =>
            this.setState({ openUnPublishConfirmationModal: false })
          }
  ...
  async handlePublicEndpointState(endpoint) {
    if (
      isStateDraft(endpoint.id, this.props.endpoints) ||
      isStateReject(endpoint.id, this.props.endpoints)
    ) {
    ...
    return (
      <WarningModal
        ...
        ignoreButtonCallback={() =>
          this.props.approve_endpoint(this.props.endpoints[this.endpointId])
        }
        onHide={() => {
        ...
        {this.props?.tabs[this.props?.activeTabId]?.status !== 'NEW' ? (
          <Dropdown className='rounded' as={ButtonGroup}>
            <button
              ...
              title={
                !this.props?.tabs[this.props?.activeTabId]?.isModified
                  ? 'No changes in this request'
                  : 'Save request'
              }
            >
              {!this.state.saveLoader ? (
                <span className='save-color'>
                  {this.props?.tabs[this.props?.activeTabId]?.isModified
                    ? 'Save'
                    : 'Saved'}
                </span>
            ...
            {getCurrentUser() ? (
              <>
                ...
                <Dropdown.Menu className=''>
                  <Dropdown.Item
                    className='px-2'
                    onClick={() => this.handleSave(null, null, null, 'example')}
                  >
  ...
  renderCodeTemplate() {
    const shouldShowCodeTemplate =
      (this.isDashboardAndTestingView(this.props, 'testing') &&
    ...
    if (shouldShowCodeTemplate && harObjectExists) {
      ...
      if (this.isDashboardAndTestingView(this.props, 'testing')) {
        return (
          <CodeTemplate {...commonProps} showClosebtn={true} theme={'light'} />
        );
      } else {
  ...
  renderEndpointUserData(isOnPublishedPage) {
    ...
    const createdAt = pages?.[currentEndpointId]?.createdAt
      ? moment(pages[currentEndpointId].createdAt).fromNow()
      : null;
    ...
    if (isOnPublishedPage) {
      ...
      return (
        <div className='d-flex justify-content-center'>
          ...
          >
            <button className='text-black-50 btn p-0 d-flex justify-content-center'>
              {' '}
              Edited {lastModified}
            </button>
          </OverlayTrigger>
    ...
    return (isDashboardRoute(this.props) &&
      ...
      >
        <div
          className={
            this.isNotDashboardOrDocView()
              ? 'mainContentWrapper dashboardPage'
              : 'mainContentWrapper d-flex'
          }
        >
          <div
            className={`innerContainer response-bottom w-100 ${!isDashboardRoute(this.props) && 'd-flex justify-content-around w-100'}`}
          >
            <div
              className={`hm-endpoint-container mid-part endpoint-container flex-column ${!isOnPublishedPage() ? 'px-3' : ''} ${this.props?.endpointContent?.currentView === 'doc'
                ? 'doc-fix-width'
                : ''
                }`}
            >
                        ...
                        <div className='d-flex gap-1 align-items-center'>
                          {this.state.showEndpointFormModal && (
                            <SaveAsSidebar
                              ...
                              name={this.props.endpointContent.data.name}
                              description={
                                this.props.endpointContent.data.description
                              }
                          ...
                          {this.props?.tabs[this.props?.activeTabId]?.status !==
                            'NEW' &&
                            !window.location.href.includes('history') && (
                              <Dropdown className='ml-1'>
                                <IconButton>
                                  <Dropdown.Toggle
                                    className='public-button p-0 text-grey'
                                    variant='default'
                                    id='dropdown-basic'
                                  >
                                    Share
                                ...
                                <Dropdown.Menu>
                                  <PublishModal
                                    onPublish={this.handleApproveEndpointRequest.bind(
                                      this
                                    )}
                                    onUnpublish={this.handleRejectEndpointRequest.bind(
                                      this
                                    )}
                                    id={endpointId}
                                    collectionId={
                                      this.props?.pages[endpointId]
                                        ?.collectionId
                                    }
                          ...
                          <Dropdown className='publish-unpublish-button'>
                            {this.props?.tabs[this.props?.activeTabId]
                              ?.status !== 'NEW' && (
              ...
              >
                <div className='endpoint-header'>
                  {!isDashboardRoute(this.props) && <PublicBreadCrumb />}
                  {this.isNotDashboardOrDocView() && (
                    <div className='d-flex endpoint-name-container justify-content-between my-3 align-items-center'>
                      {this.isNotDashboardOrDocView() && (
                        <>
                          <h1 className='font-weight-bold border-0 d-flex align-items-center mt-0'>
                          ...
                          {!isDashboardRoute(this.props) && (
                            <div className='request-button'>
                              <button
                                ...
                                style={{
                                  backgroundColor:
                                    this.props.publicCollectionTheme,
                                  opacity: 0.9,
                                }}
                                type='submit'
                ...
                <div
                  className={
                    this.isNotDashboardOrDocView() ? 'hm-panel' : 'hm-panel'
                  }
                >
                  {this.isDashboardAndTestingView() && (
                            ...
                            <>
                              ...
                              >
                                <div>
                                  <Authorization
                                    {...this.props}
                                    set_authorization_headers={this.setHeaders.bind(
                                      this
                                    )}
                                    set_authoriztaion_params={this.setParams.bind(
                                      this
                                    )}
                                    set_authoriztaion_type={this.setAuthType.bind(
                                      this
                                    )}
                                    handleSaveEndpoint={this.handleSave.bind(
                                      this
                                    )}
                                    delete_headers={this.deleteHeader.bind(
                                      this
                                    )}
                              ...
                              >
                                <ul
                                  className='nav nav-tabs mt-0 border border-0 script-button-wrapper rounded mb-2'
                                  id='pills-sub-tab'
                                  role='tablist'
                                >
                                  <li className='nav-item'>
                                ...
                                >
                                  ...
                                  >
                                    <div>
                                      <Script
                                        type='Pre-Script'
                                        handleScriptChange={this.handleScriptChange.bind(
                                          this
                                        )}
                                        scriptText={
                                          this.props?.endpointContent
                            ...
                            <>
                              ...
                              >
                                <QueryTab
                                  ...
                                  endpointContent={this.props.endpointContent}
                                  setQueryTabBody={this.setQueryTabBody.bind(
                                    this
                                  )}
                              ...
                              >
                                <ul
                                  className='nav nav-tabs mt-0 script-button-wrapper rounded mb-2 border border-0'
                                  id='pills-sub-tab'
                                  role='tablist'
                                >
                                  <li className='nav-item'>
                          ...
                          >
                            <div>
                              <Script
                                ...
                                scriptText={
                                  this.props?.endpointContent?.preScriptText
                                }
                                endpointContent={this.props?.endpointContent}
                              />
                            </div>
                          ...
                          >
                            <div>
                              <Script
                                ...
                                scriptText={
                                  this.props?.endpointContent?.postScriptText
                                }
                                endpointContent={this.props?.endpointContent}
                              />
                            </div>
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(withQuery(DisplayEndpoint))
);
