...
} from '../common/authorizationEnums';
...
export default function Authorization(props) {
  ...
  const [selectedAuthorizationType, setSelectedAuthorizationType] = useState(
    authorizationTypes[
      endpointStoredData?.authorizationData?.authorizationTypeSelected
    ] || authorizationTypes.noAuth
  );
  const [addAuthorizationDataToForAuth2, setAddAuthorizationDataToForAuth2] =
    useState(
      addAuthorizationDataTypes[
        endpointStoredData?.authorizationData?.authorization?.oauth2
          ?.addAuthorizationRequestTo
      ] || addAuthorizationDataTypes.select
    );
  const [basicAuthData, setBasicAuthData] = useState({
    username:
      endpointStoredData?.authorizationData?.authorization?.basicAuth
        ?.username || '',
    password:
      endpointStoredData?.authorizationData?.authorization?.basicAuth
        ?.password || '',
  ...
  const [selectedTokenValue, setSelectedTokenValue] = useState(
    tokenDetails?.[selectedTokenId]?.accessToken || ''
  );

  useEffect(() => {
    if (endpointStoredData?.authorizationData) {
      const authTypeSelected =
        endpointStoredData.authorizationData.authorizationTypeSelected;
      const newAuthType =
        authorizationTypes[authTypeSelected] || authorizationTypes.noAuth;
  ...
  const handleSelectAuthorizationType = (key) => {
    const endpointStoredData =
      params.endpointId !== 'new'
        ? queryClient.getQueryData(queryKey)
        : tabs.tabs?.[activeTabId]?.draft || {};
  ...
  const handleAddAuthorizationTo = (key) => {
    ...
    if (
      ...
      addAuthorizationDataTypes[key] === addAuthorizationDataTypes.requestUrl
    ) {
  ...
  const addAccessTokenInsideHeadersAndParams = (value, tokenIdToSave) => {
    if (
      ...
      addAuthorizationDataToForAuth2 === addAuthorizationDataTypes.requestUrl
    ) {
      props.set_authoriztaion_params(
        value,
        'access_token',
        null,
        tokenIdToSave
  ...
  return (
    <div className='authorization-panel'>
            ...
            >
              {Object.keys(authorizationTypes).map((key, index) => (
                <button
                  ...
                  onClick={() => handleSelectAuthorizationType(key)}
                  key={index}
                >
                ...
                >
                  {Object.keys(addAuthorizationDataTypes).map((key, index) => {
                    return (
                      <button
                        key={index}
                        onClick={() => handleAddAuthorizationTo(key)}
                        className='dropdown-item'
                      >
                        {addAuthorizationDataTypes[key]}
                      </button>
      ...
      {selectedAuthorizationType === authorizationTypes.oauth2 && (
        <Auth2Configurations
          {...props}
          addAccessTokenInsideHeadersAndParams={
            addAccessTokenInsideHeadersAndParams
          }
