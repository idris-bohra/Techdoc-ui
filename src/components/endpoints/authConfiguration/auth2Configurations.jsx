...
export default function Auth2Configurations(props) {
  ...
  const handleSelectToken = (tokenId) => {
    ...
    props.addAccessTokenInsideHeadersAndParams(
      tokenDetails?.[tokenId]?.accessToken,
      tokenId
    );
  };
      ...
      <div className='authorization-editor-wrapper'>
        <form>
          <div className='input-field-wrapper form-group d-block mb-1'>
            <div>
              <label className='basic-auth-label text-secondary'>
                Access Token
              </label>
            </div>
            <div className='basic-auth-input'>
              <input
                ...
                value={props?.selectedTokenValue ?? ''}
                name='accessToken'
              ...
              <div className='dropdown available-token-dropdown ml-2'>
                ...
                >
                  ...
                  tokenDetails[props?.selectedTokenId]?.tokenName
                    ? tokenDetails[props?.selectedTokenId]?.tokenName
                    : 'Available Tokens'}
                </button>
                <div
                  className='dropdown-menu dropdown-menu-token available-token-dropdown-menu'
                ...
                >
                  {Object.keys(tokenDetails).map((tokenId, index) => (
                    <button
                      ...
                      key={index}
                      type='button'
                      className='dropdown-item dropdown-token-item  px-1 font-12 d-block'
                    >
                  ...
                  {Object.keys(tokenDetails)?.length !== 0 ? (
                    <button
                      onClick={handleManageTokenClick}
                      type='button'
                      className='dropdown-item manage-token-title font-12 px-1 d-block'
                    >
                      Manage Tokens
                    </button>
                  ) : (
          ...
          <div className='input-field-wrapper d-block'>
            <div className='basic-auth-input'>
              <IconButton variant='sm'>
                <button
                  className='btn font-12 text-grey'
                  type='button'
