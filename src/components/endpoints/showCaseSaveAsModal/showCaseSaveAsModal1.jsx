...
const ShowCaseSaveAsModal = (props) => {
  ...
  const { collections, currentOrg, pages, draftContent } = useSelector(
    (state) => {
      return {
  ...
  const handleSave = () => {
    ...
    if (pages?.[currentId]?.type === 1)
      rootParentId = pages?.[currentId]?.child?.[0];
      ...
      <div className='showcase_modal_container'>
        ...
        <div className='mt-5 d-flex align-items-center justify-content-end pb-2 pr-1'>
          <button
            ...
            disabled={currentId === 'currentOrganisation'}
          >
            Save
          </button>
          <button
            onClick={props.onHide}
            className='btn btn-secondary outline font-12 api-cancel-btn btn-sm'
          >
            Cancel
          </button>
        </div>
