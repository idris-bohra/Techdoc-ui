...
const CodeTemplate = (props) => {
  ...
  const makeCodeSnippet = async (newSelectedLanguage) => {
    ...
    try {
      ...
      const language =
        newSelectedLanguage === 'axiosNode' ? 'node' : newSelectedLanguage;
  ...
  const getClassForLanguages = (key) => {
    const commonClass =
      'd-flex d-md-flex flex-column justify-content-center align-items-center';
    let classToReturn =
      key === selectedLanguage ? 'active ' + commonClass : commonClass;
  ...
  return (
    <>
      <div
        className={`${isOnPublishedPage() ? 'position-sticky h-100' : 'sample-code-inner position-sticky top-0'}`}
      >
        <div
          className={
            params.endpointId
        ...
        >
          <div
            className={`inner-editor ${isOnPublishedPage() ? 'border-none' : 'pt-3 border-left'}`}
          >
            <Col
              id='code-window-sidebar'
              xs={12}
              className='px-3 pb-1 d-flex flex-column gap-4 responsive-padding'
            >
              <div className='code-heading d-flex align-items-center'>
                <span
                  className={
                    props.theme === 'light' ? 'col-black' : 'col-black'
                  }
                >
                  Sample code
                </span>
              ...
              <div className='select-code-wrapper d-flex align-items-center gap-1 mb-3 img'>
                ...
                <Dropdown className='dropdown-more'>
                  <Dropdown.Toggle
                    className={
                      secondaryLanguages.includes(selectedLanguage)
                        ? 'active dropdownMore mr-0'
                        : 'dropdownMore mr-0'
                    }
                  >
            ...
            <Col
              className={
                isOnPublishedPage()
                  ? 'editor-body-wrapper'
                  : 'editor-body-wrapper editor-body-wrapper-height'
              }
              xs={12}
            >
              <div className='ace-editor-wrapper responsive-padding'>
                ...
                <div
                  id='code-window-body'
                  className={
                    !isOnPublishedPage() ? 'empty-div-light' : 'empty-div-dark'
                  }
                ></div>
              </div>
            </Col>
