...
export function apiTest(api, method, body, headers, bodyType, cancelToken) {
  if (api.indexOf('localhost') > 0 || api.indexOf('127.0.0.1') > 0) {
    ...
    return httpService.post(`${apiUrlEndpoint}/p/test-apis/run`, data, {
      cancelToken,
    });
  }
}
...
export function addExampleRequest(endpointId, endpoint = null) {
  const apiUrl = getApiUrl();
  return http.post(
    `${apiUrl}/endpoints/sample-request/${endpointId}`,
    endpoint
  );
}
...
export async function getTokenAuthorizationCodeAndAuthorizationPKCE(
  accessTokenURL,
  code,
  data
) {
