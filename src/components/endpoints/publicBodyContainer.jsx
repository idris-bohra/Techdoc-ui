...
class PublicBodyContainer extends Component {
        ...
        <div className='d-flex flex-row-reverse'>
          {types.map((type, index) => (
            <small key={index} className='ml-3 text-small'>
              <Badge className={`body-desc-type ${type}`}>
                {type.charAt(0)}
              </Badge>{' '}
        ...
        if (['object', 'array'].includes(value.type)) {
          return (
            <div
              key={newPath}
              className='ml-2 pl-2'
              style={{ borderLeft: '1px solid rgb(0,0,0,0.1)' }}
            >
              <strong className='pl-1' style={{ cursor: 'default' }}>
                {value.type === 'object' ? (
                  <Badge className='body-desc-type object'>O</Badge>
                ) : null}
                {value.type === 'array' ? (
                  <Badge className='body-desc-type array'>A</Badge>
                ) : null}{' '}
                {key}
              </strong>
              {value.type === 'object'
                ? renderObject(newPath, value.value)
                : null}
              {value.type === 'array'
                ? renderArray(newPath, value.value)
                : null}
            </div>
          );
        } else {
        ...
        if (['object', 'array'].includes(item.type)) {
          return (
            ...
            >
              ...
              {item.type === 'object'
                ? renderObject(newPath, item.value)
                : null}
  ...
  render() {
    ...
    if (this.props.body && this.props.endpointContent?.protocolType == 2)
      return this.graphqlBody();
    if (this.props.body && this.props.body.type === 'none') return null;
    return (
      <>
        {this.props.body &&
          this.props.body.type === bodyTypesEnums['multipart/form-data'] &&
          this.props?.body?.[bodyTypesEnums['multipart/form-data']]?.length >
            0 && (
            <GenericTable
              {...this.props}
              title='formData'
        ...
        {this.props.body?.[
          ...
          this.props.body.type ===
            bodyTypesEnums['application/x-www-form-urlencoded'] &&
          this.props.body?.[bodyTypesEnums['application/x-www-form-urlencoded']]
            ?.length > 0 && (
            <GenericTable
              {...this.props}
        ...
        {this.props.body?.[bodyTypesEnums['multipart/form-data']] &&
          ...
          (this.props.body.type === rawTypesEnums.JSON ? (
                ...
                <div className='d-flex justify-content-between'>
                  <ul className='public-endpoint-tabs'>
                    <li
                      ...
                      onClick={() =>
                        this.setState({ showBodyCodeEditor: true })
                      }
                    >
                      Raw
                    </li>
                    <li
                      ...
                      onClick={() =>
                        this.setState({ showBodyCodeEditor: false })
                      }
                    >
                      Body description
                    </li>
              ...
              {this.state.showBodyCodeEditor ? (
                <div className='body-ace-editer' onClick={this.toggleEditor}>
                  <div
                    ...
                    className='custom-editor-public-page'
            ...
            <div className='hm-public-table'>
              <div className='public-generic-table-title-container'>
                Body{' '}
                <small className='text-muted'>({this.props.body.type})</small>
                {willHighlight(this.props, 'body') ? (
                  <i className='fas fa-circle' />
                ) : null}
              </div>
              <AceEditor
                ...
                onChange={(value) =>
                  this.props.set_body(this.props.body.type, value)
                }
                setOptions={{
                  showLineNumbers: true,
                }}
