...
class BodyContainer extends Component {
  ...
  handleSelectBodyType(bodyType, bodyDescription) {
    ...
    if (bodyType === bodyTypesEnums['raw'] && bodyDescription) {
      ...
      this.props.set_body(
        this.state.selectedRawBodyType,
        this.state.data.raw,
        this.state.selectedRawBodyType || 'TEXT'
      );
    } else {
      this.flag = false;
            ...
            <>
              <div className='prettify-button position-absolute'>
                <IconButton>
                  <span
                    className='text-grey'
                    onClick={this.handlePrettifyJson.bind(this)}
                  >
                    Beautify
                  </span>
                </IconButton>
  ...
  matchCurrentBodyType(bodyType) {
    if (
      this.props.body &&
      this.props.body.type + '-' + this.props.endpoint_id === bodyType
    )
      return true;
    return false;
  }
      ...
      <React.Fragment>
        <div className='button-panel-wrapper'>
          <form className='body-select d-flex align-items-center mb-2 ml-3'>
            <label className='customRadio'>
              <input
                ...
                onClick={() =>
                  this.handleSelectBodyType(bodyTypesEnums['none'])
                }
            ...
            <label className='customRadio'>
              <input
                ...
                onClick={() =>
                  this.handleSelectBodyType(
                    bodyTypesEnums['multipart/form-data']
                  )
                }
                onChange={() => {}}
                className='custom-radio-input'
                checked={this.matchCurrentBodyType(
                  `${bodyTypesEnums['multipart/form-data']}-${this.props.endpoint_id}`
                )}
              />
            ...
            <label className='customRadio'>
              <input
                ...
                onChange={() => {}}
                checked={this.matchCurrentBodyType(
                  `${bodyTypesEnums['application/x-www-form-urlencoded']}-${this.props.endpoint_id}`
                )}
              />
          ...
          {isSavedEndpoint(this.props) &&
            this.state.selectedRawBodyType === rawTypesEnums.JSON &&
            (this.state.selectedBodyType === bodyTypesEnums['raw'] ||
              this.state.selectedBodyType === rawTypesEnums.JSON) && (
              <div
                className='btn-group btn-group-toggle customBtnGroup mb-2'
                data-toggle='buttons'
                style={{ float: 'right' }}
              >
                <label
                ...
                >
                  <input
                    ...
                    defaultChecked
                    onClick={() =>
                      this.handleSelectBodyType(bodyTypesEnums['raw'])
                    }
                  />
                  Raw
                </label>
