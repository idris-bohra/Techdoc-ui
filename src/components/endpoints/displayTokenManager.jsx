  ...
  return (
    ...
    >
      ...
      <Row className='p-0 m-0 content-container'>
          ...
          <ListGroup>
            ...
            {Object.keys(tokenDetails).map((tokenId, index) => (
              <div
                ...
                className={`token-name-container ${selectedTokenId === tokenId && 'selected-token-state'}`}
                key={index}
              >
                <ListGroup.Item className='tokens-list-item ml-2 d-flex justify-content-between align-items-center'>
                  ...
                  <MdDeleteOutline
                    onClick={(e) => handleDeleteTokenClick(e, tokenId)}
        ...
        <Col
          className='mt-2 mb-2 token-details-field-container d-flex flex-column justify-content-between'
          sm={9}
        >
          {selectedTokenId && (
            <>
              ...
              <button
                ...
                className='oauth2-token-details-list-item-button font-12 ml-auto btn btn-secondary'
                type='button'
              >
                Use Token
              </button>
  ...
  return (
    <div className='h-100 mt-3'>
      {tokenDetailsToShow?.map((key) => {
        ...
        if (
          key === 'clientSecret' &&
          tokenDetails?.[selectedTokenId]?.grantType ===
            grantTypesEnums.implicit
        )
          return null;
        if (
          key === 'accessTokenUrl' &&
