...
function TokenGenerator(props) {
  ...
  const endpointId =
    params.endpointId !== 'new' ? params.endpointId : activeTabId;
  ...
  const [data, setData] = useState({
    tokenName:
      endpointStoredData?.authorizationData?.authorization?.oauth2?.tokenName ||
      'Token Name',
    selectedGrantType:
      endpointStoredData?.authorizationData?.authorization?.oauth2
        ?.selectedGrantType || grantTypesEnums.authorizationCode,
    callbackUrl: `${process.env.NEXT_PUBLIC_UI_URL}/auth/redirect` || '',
    authUrl:
      endpointStoredData?.authorizationData?.authorization?.oauth2?.authUrl ||
      '',
    username:
      endpointStoredData?.authorizationData?.authorization?.oauth2?.username ||
      '',
    password:
      endpointStoredData?.authorizationData?.authorization?.oauth2?.password ||
      '',
    accessTokenUrl:
      endpointStoredData?.authorizationData?.authorization?.oauth2
        ?.accessTokenUrl || '',
    clientId:
      endpointStoredData?.authorizationData?.authorization?.oauth2?.clientId ||
      '',
    clientSecret:
      endpointStoredData?.authorizationData?.authorization?.oauth2
        ?.clientSecret || '',
    scope:
      endpointStoredData?.authorizationData?.authorization?.oauth2?.scope || '',
    state:
      endpointStoredData?.authorizationData?.authorization?.oauth2?.state || '',
    clientAuthentication:
      endpointStoredData?.authorizationData?.authorization?.oauth2
        ?.clientAuthentication || clientAuthenticationTypeEnums?.sendOnHeaders,
    codeMethod:
      endpointStoredData?.authorizationData?.authorization?.oauth2
        ?.codeMethod || codeMethodTypesEnums.sh256,
    codeVerifier:
      endpointStoredData?.authorizationData?.authorization?.oauth2
        ?.codeVerifier || '',
    refreshTokenUrl:
    ...
    function receiveMessage(event) {
      if (event?.data && event?.data?.techdocAuthenticationDetails) {
        getAuthenticationDetails(
          event?.data?.techdocAuthenticationDetails,
          dataRef.current
        );
      }
    }
  ...
  const getAuthenticationDetails = async (authDetail, configurationDetails) => {
    ...
    if (
      configurationDetails.selectedGrantType ===
      ...
      try {
        accessTokenData =
          await endpointApiService.getTokenAuthorizationCodeAndAuthorizationPKCE(
      ...
      configurationDetails.selectedGrantType === grantTypesEnums.implicit
    ) {
      if (!authDetail?.implicitDetails?.access_token)
  ...
  const getCurrentTimeOfTokenGeneration = () => {
    ...
    dateTimeOfTokenGeneration.setSeconds(
      dateTimeOfTokenGeneration.getSeconds() - 20
    );
  ...
  const getAcessTokenForPasswordAndClientGrantType = async () => {
    try {
      const responseData =
        await endpointApiService.getTokenPasswordAndClientGrantType(
          data.accessTokenUrl,
          dataRef.current
        );
      const createdTime = getCurrentTimeOfTokenGeneration();
  ...
  async function makeRequest() {
    ...
    var options =
      'width=200,height=200,resizable=yes,scrollbars=yes,status=yes';
  ...
  function handleSaveConfiguration() {
    if (data.tokenName?.length > 25)
      return toast.error('Token Name character limit is 25');
              ...
              >
                {Object.keys(grantTypesEnums).map((key) => (
                  <button
                    ...
                    onClick={() => handleGrantTypeClick(key)}
                  >
                    {grantTypesEnums[key]}
                  </button>
                ))}
        ...
        return (
          <>
            ...
            <div className='dropdown basic-auth-input'>
              ...
              >
                <button
                  value={clientAuthenticationTypeEnums.sendOnHeaders}
                  className='dropdown-item'
                  onClick={setClientAuthorization}
                >
                  {clientAuthenticationTypeEnums.sendOnHeaders}
                </button>
                <button
                  value={clientAuthenticationTypeEnums.sendOnBody}
                  className='dropdown-item'
                  onClick={setClientAuthorization}
                >
          ...
          >
            {Object.keys(codeMethodTypesEnums).map((key) => (
              <button
                key={key}
                className='dropdown-item'
                onClick={() => handleCodeMethodClick(codeMethodTypesEnums[key])}
              >
  ...
  return (
    <Modal onHide={closeModal} id='modal-new-token-generator' className='get-access-token' size='lg' animation={false} aria-labelledby='contained-modal-title-vcenter' centered show={props?.show}>
      ...
      <Modal.Header className='custom-collection-modal-container d-flex justify-content-between align-items-center'>
        ...
        <IconButton onClick={closeModal}><MdOutlineClose size={22} /></IconButton>
      </Modal.Header>
      ...
      <Modal.Body className='new-token-generator-modal-body'>
        ...
        <div className='text-right'>
          <button
            className='btn btn-secondary outline btn-sm font-12 ml-2'
            onClick={handleSaveConfiguration}
          >
            Save
          </button>
          <button
            className='btn btn-primary btn-sm font-12 ml-2'
            type='button'
            onClick={makeRequest}
          >
            Request Token
