...
export class Script extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      this.props.scriptText &&
      !this.scriptFetched &&
      this.props.scriptText !== prevProps.scriptText
    ) {
      this.setState({ scriptEditorText: this.props.scriptText || '' });
  ...
  insertSnippet(snippet) {
    ...
    const updatedScriptText =
      scriptEditorText.substring(
        0,
        editor.session.getDocument().positionToIndex(nextLinePosition)
      ) +
      snippetText +
      scriptEditorText.substring(
        editor.session.getDocument().positionToIndex(nextLinePosition)
    ...
    editor.gotoLine(
      endOfInsertedSnippetPosition.row + 1,
      endOfInsertedSnippetPosition.column
    );
    ...
    return (
      <div className='snippets col-4'>
        ...
        {snippets.map((snippet, index) => (
          <div
            key={index}
            onClick={() => this.insertSnippet(Snippets[snippet])}
          >
