...
function GenericTableAutoSuggest(props) {
  ...
  useEffect(() => {
    setTimeout(() => {
      if (document.activeElement !== genericAutoSuggestRef?.current)
        sethtmlData(props?.htmlValue || '');
    }, 0);
  ...
  const handleValueChange = () => {
    ...
    const value =
      genericAutoSuggestRef.current.innerHTML === '<br>'
        ? ''
        : genericAutoSuggestRef.current.innerHTML;
    props.handleChange(value, { name: props?.valueKey, value: value });
    setTimeout(
      () => restoreCaretPosition(genericAutoSuggestRef.current, caretPosition),
      0
    );
  };
