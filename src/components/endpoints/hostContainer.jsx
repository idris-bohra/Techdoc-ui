...
class HostContainer extends Component {
  constructor(props) {
    ...
    this.debouncedHandleInputHostChange = debounce(
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (!_.isEqual(prevProps.URL, this.props?.URL)) {
      if (document.activeElement !== this.hostcontainerRef?.current)
        this.setState({ initalUrlValue: this.props?.URL });
    }
    if (
      !_.isEqual(
        prevProps?.endpointContent?.host?.BASE_URL,
        this.props?.endpointContent?.host?.BASE_URL
      )
    ) {
      this.setState({
        datalistHost: this.props?.endpointContent?.host?.BASE_URL,
  ...
  handleClickOutside(event) {
    if (
      (this.state.showDatalist || this.state.showInputHost) &&
      this.wrapperRef &&
  ...
  setHostAndUri() {
    ...
    const host =
      this.props?.endpointContent?.host?.BASE_URL ||
      this.state?.datalistHost ||
      '';
    this.setState({ datalistUri: endpointUri, datalistHost: host }, () =>
  ...
  getDataFromParsedData(untitledEndpointData, parsedData) {
    ...
    untitledEndpointData.data.name =
      this.props.endpointContent?.data?.name || 'Untitled';
    untitledEndpointData.currentView =
    ...
    if (parsedData.auth_type == 'basic') {
      ...
      untitledEndpointData.authorizationData.authorizationTypeSelected =
        parsedData.auth_type;
    }
    ...
    if (parsedData?.data) {
      ...
      if (contentType.includes('application/json')) {
        ...
        untitledEndpointData.data.body.raw.rawType =
          contentTypesEnums[contentType];
        untitledEndpointData.data.body.raw.value =
      ...
      // setting data for all the rawTypes defined except JSON
        ...
        parsedData.headers['Content-Type'] = !parsedData.headers?.[
          'Content-Type'
        ]
          ? parsedData.headers?.['content-type']
          : parsedData.headers?.['Content-Type'];
        let bodyType = (untitledEndpointData.data.body.type = !parsedData
        ...
        untitledEndpointData.data.body[bodyType].push(
          ...untitledEndpointData.data.body[bodyType].splice(0, 1)
        );
      }
    }
    ...
    untitledEndpointData.originalHeaders.push(
      ...untitledEndpointData.originalHeaders.splice(0, 1)
    );
    ...
    untitledEndpointData.originalParams.push(
      ...untitledEndpointData.originalParams.splice(0, 1)
    );
  ...
  async handleInputHostChange() {
    if (!this.hostcontainerRef || !this.hostcontainerRef.current) {
      console.error(
        'hostcontainerRef is not set or does not have a current value'
      );
      return;
    }
    ...
    if (inputValue.trim().startsWith('curl')) {
      try {
        ...
        parsedData.url = parsedData.url.replace(
          /^(http:\/\/https?:\/\/)/,
          '$2'
        );
        parsedData.raw_url = parsedData.raw_url.replace(/^http:\/\/\s/, '');
        parsedData.raw_url = parsedData.raw_url.replace(
          /^(http:\/\/https?:\/\/)/,
          '$2'
        );
        this.getDataFromParsedData(this.props.untitledEndpointData, parsedData);
        this.hostcontainerRef.current.innerHTML = convertTextToHTML(
          parsedData.raw_url
        );
        return;
      } catch (e) {
  ...
  checkExistingHosts(value) {
    ...
    if (
      environmentHost &&
      value?.match(new RegExp('^' + environmentHost) + '/')
    ) {
