...
const History = () => {
  ...
  const renderName = (history) => {
    const baseUrl = history?.endpoint?.BASE_URL
      ? history?.endpoint?.BASE_URL + history?.endpoint?.uri
      : history?.endpoint?.uri;
    ...
    return (
      Object.keys(history)?.length !== 0 && (
        ...
        >
          ...
          {history?.endpoint?.protocolType === 2 && (
  ...
  const renderHistoryList = () => {
    ...
    historySnapshot.forEach((history) => {
      ...
      if (today.isSame(createdAtMoment, 'day')) {
        ...
        createdAtMoment.isSame(today.clone().subtract(1, 'days'), 'day')
      ) {
        dateGroup = 'Yesterday';
      } else {
    ...
    const sortedGroupedHistory = Object.entries(groupedHistory).sort(
      ([dateGroupA], [dateGroupB]) => {
        if (dateGroupA === 'Today') return -1;
        if (dateGroupB === 'Today') return 1;
        if (dateGroupA === 'Yesterday') return -1;
        if (dateGroupB === 'Yesterday') return 1;
        return moment(dateGroupB, ['MMMM D, YYYY']).diff(
          moment(dateGroupA, ['MMMM D, YYYY'])
        );
      }
    );
