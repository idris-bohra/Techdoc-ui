...
export const addParentPageVersion = (newVersion, pageId, customCallback) => {
  newVersion.uniqueTabId = sessionStorage.getItem(
...
export const onDefaultVersion = (orgId, versionData) => {
  versionData.uniqueTabId = sessionStorage.getItem(
    SESSION_STORAGE_KEY.UNIQUE_TAB_ID
  );
  return (dispatch) => {
    collectionVersionsApiService
      ...
      .catch((error) => {
        dispatch(
          onSetDefaultVersionError(error.response ? error.response.data : error)
        );
      });
  };
};
