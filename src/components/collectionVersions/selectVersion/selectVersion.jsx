...
const VersionInput = (props) => {
  ...
  const onRename = (versionId) => {
    ...
    try {
      if (versionNameInputRef.current.value.trim()?.length === 0)
        return toast.error('Name cannot be empty');
      versionChilds.forEach((element) => {
        if (
          versionId !== element &&
          pages[element]?.name.trim().toLowerCase() ===
            versionNameInputRef.current.value.trim().toLowerCase()
        ) {
      ...
      {props?.showEdit === props?.index ? (
        <OutsideClickHandler onOutsideClick={handleOutsideClickOfInputField}>
          <div className='d-flex justify-content-start align-items-center'>
            ...
            <Button
              ...
              variant='btn btn-outline btn-sm font-12 ml-2'
              onClick={() => onRename(props?.singleChildId)}
            >
              Save
            </Button>
          </div>
        ...
        <div className='d-flex justify-content-start align-items-center'>
          <div className='version-title'>
            {pages?.[props?.singleChildId]?.name}
          </div>
          <BiSolidPencil
            size={14}
          ...
          />
          {pages[props?.singleChildId]?.state === 1 && (
...
const AddVersion = (props) => {
  ...
  const addVersion = () => {
    if (newVersionNameInputRef.current.value.trim()?.length === 0)
      return toast.error('Name cannot be empty');
    const versionChilds = pages?.[props?.parentPageId]?.child;
    try {
      versionChilds.forEach((element) => {
        if (
          pages[element]?.name.trim().toLowerCase() ===
          newVersionNameInputRef.current.value.trim().toLowerCase()
        ) {
          throw new Error('StopIteration');
        }
      });
