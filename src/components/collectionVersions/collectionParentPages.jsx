...
const CollectionParentPages = (props) => {
    ...
    return (
      showAddCollectionModal && (
        <DefaultViewModal
          ...
          selectedVersion={
            selectedVersionId ? selectedVersionId : defaultVersionId
          }
          pageType={3}
        />
      )
  ...
  const openManageVersionModal = () => {
    return (
      showVersionForm && (
        <CustomModal
          modalShow={showVersionForm}
          onHide={() => setShowVersionForm(false)}
        >
  ...
  const handleRedirect = (id) => {
    if (isDashboardRoute({ location }))
      return router.push(`/orgs/${params.orgId}/dashboard/page/${id}`);
  ...
  const versionName = () => {
    const versionName =
      defaultVersionName?.length > 10
        ? `${defaultVersionName.substring(0, 7)} ... `
        : defaultVersionName;
      ...
      return (
        ...
        >
          {pages[rootId].child.map((childId, index) => (
            <Dropdown.Item
              key={index}
              onClick={(e) => handleDropdownItemClick(childId, rootId)}
            >
              {pages[childId]?.name}
            </Dropdown.Item>
          ))}
  ...
  const renderBody = (pageId) => {
    ...
    const isSelected =
      ...
      sessionStorage.getItem('currentPublishIdToShow') === pageId
        ? 'selected'
    ...
    return (
      <div className={'hm-sidebar-outer-block my-1'} key={pageId}>
        ...
        >
          <button
            tabIndex={-1}
            className={`px-0 ${expanded ? 'expanded' : ''}`}
          >
            <div
              className={`active-select d-flex align-items-center justify-content-between rounded my-1 ${isSelected ? ' selected text-dark' : ''} text-secondary `}
              style={backgroundStyle}
              onMouseEnter={() => setIsHovered(true)}
              onMouseLeave={() => setIsHovered(false)}
            >
              <div
                className={`d-flex align-items-end cl-name `}
                onClick={(e) => handleParentPageClick(e, expanded)}
              >
                <div className='d-flex td-name ml-1 align-items-center'>
                  <span
                    className={`versionChovron icon-header d-flex justify-content-center`}
                  ...
                  <div
                    ...
                    style={
                      props.draggingOverId === pageId
                        ? { border: '3px solid red' }
                        : null
                    }
                  >
                    <div className={`text-truncate d-inline fw-500 `}>
                      {pages[pageId]?.name}
                    </div>
              ...
              {isDashboardRoute({ location }, true) &&
              !collections[props.collection_id]?.importedFromMarketPlace ? (
                <div className='sidebar-item-action align-items-center'>
                  <div
                    className='d-flex align-items-center'
                    onClick={() =>
                      openAddPageEndpointModal(
                  ...
                  <div className='dropdown-menu dropdown-menu-right'>
                    <div
                      ...
                      onClick={() => openEditPageForm(pageId)}
                    >
                      <FiEdit2 color='var(--icon-color-dark-grey)' size={16} />{' '}
                      Rename
                    </div>
                    {isOrgDocType() && (
                      <div
            ...
            <div className='version-collapse'>
              <Card.Body>
                <div className='linkWrapper versionPages'>
                  {pages[
                    pages[props.rootParentId].child?.length === 1
                      ? defaultVersionId
                      : selectedVersionId
                  ]?.child?.length !== 0 ? (
                    <CombinedCollections
