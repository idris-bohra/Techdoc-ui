...
const Environments = () => {
  const { environment, currentEnvironmentId, organizations } = useSelector(
    (state) => {
      return {
  ...
  const handleEnvironmentModal = (
    newEnvironmentFormName,
    newEnvironmentToBeEdited
  ) => {
  ...
  const handleEyeClick = () => {
    if (environment?.environments[currentEnvironmentId])
      handleEnvironmentModal(
        'Edit Environment',
        environment.environments[currentEnvironmentId]
      );
  };
  if (organizations?.currentOrg?.meta?.type !== 0) {
    return (
      <div className='environment-container d-flex align-items-center transition'>
        {(environmentFormName === 'Add new Environment' ||
          environmentFormName === 'Edit Environment') && (
          <EnvironmentVariables
            show
            onHide={handleEnvironmentModal}
            title={environmentFormName}
            environment={environmentToBeEdited}
          />
        )}
        ...
        {showDeleteModal && (
          <DeleteModal
            show
            onHide={closeDeleteEnvironmentModal}
            title={'Delete Environment'}
            message={'Are you sure you wish to delete this environment?'}
        ...
        <div
          ...
          className={`environment-buttons addEniButton ${environment.environments[currentEnvironmentId] && 'hover'}`}
        >
          <IconButton>
            {environment?.environments[currentEnvironmentId] ? (
              <EyeIcon className='cursor-pointer m-1' />
            ) : (
              <EyeDisabledIcon className='m-1' />
            )}
          </IconButton>
        </div>
          ...
          <Dropdown className='ml-1'>
            ...
            <Dropdown.Menu alignRight>
              ...
              {Object.keys(environment.environments).map((environmentId) => (
                <Dropdown.Item
                  className='gap-2 px-2 align-items-end dropdown-item-icons justify-content-between'
                  onClick={() => handleEnv(environmentId)}
                  key={environmentId}
                >
                  ...
                  >
                    <BiExport
                      ...
                      onClick={(event) => {
                        ...
                        handleExport(
                          environment?.environments[environmentId].id
                        );
                      }}
                    />
                  </OverlayTrigger>

                  {environment?.environments[environmentId]?.userId === 0 &&
                    environment?.environments[environmentId]?.orgId !== null &&
                    environmentId !== currentEnvironmentId && (
              ...
              <Dropdown.Item
                ...
                onClick={() => handleEnvironmentModal('Add new Environment')}
              >
                Add Environment
              </Dropdown.Item>
              <Dropdown.Item
              ...
              <Dropdown.Item
                className='dropdown-item px-2'
                onClick={() => handleEnvironmentModal('Environment modal')}
              >
                Manage Environment
              </Dropdown.Item>
            </Dropdown.Menu>
