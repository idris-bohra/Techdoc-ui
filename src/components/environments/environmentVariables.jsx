...
const EnvironmentVariables = ({
  ...
  const [originalVariableNames, setOriginalVariableNames] = useState([
    'BASE_URL',
    '1',
  ]);
  const [updatedVariableNames, setUpdatedVariableNames] = useState([
    'BASE_URL',
    '',
  ]);
  ...
  useEffect(() => {
    ...
    if (
      initialEnvironment &&
      Object.keys(initialEnvironment.variables)?.length > 0
    ) {
  ...
  const schema = {
    ...
    type:
      title === 'Edit Environment'
        ? Joi.number().optional()
        : Joi.number().required().label('Environment Type'),
  };
  ...
  const doSubmit = () => {
    ...
    const existingEnvironment =
      initialEnvironment && initialEnvironment.name === environment.name;
    ...
    if (title === 'Add new Environment' && !existingEnvironment) {
      ...
      if (environment.id) {
        if (
          JSON.stringify(originalEnvCopy) !== JSON.stringify(updatedEnvironment)
        ) {
          dispatch(
            updateEnvironment({
              id: environment.id,
      ...
      >
        <form onSubmit={handleSubmit}>
          <div className='custom-environment-modal-container'>
            <Modal.Header
              className='custom-collection-modal-container p-3'
              closeButton
            >
              ...
              <div className='form-group'>
                ...
                <div>
                  <small className='muted-text'>
                    *environment name accepts min 3 and max 50 characters
                  </small>
                </div>
                {errors?.name && (
                        ...
                        <tr key={index}>
                          ...
                          {updatedVariableNames?.length - 1 !== index && (
                            <td className='custom-td'>
                              <button
                                type='button'
                                className='btn btn-light btn-sm btn-block env-delete-icon'
                                onClick={() => handleDelete(index)}
                              >
                                <MdDeleteOutline
                                  className='text-grey rounded p-1'
                                  size={24}
                                />
                              </button>
                            </td>
            ...
            <div className='custom-table-footer p-3'>
              <div className='text-left'>
                <button
                  className='btn btn-primary btn-sm font-12 mr-2'
                  id='add_env_save_btn'
                >
                  Save
                </button>
                <button
                  className='btn btn-secondary outline btn-sm font-12'
                  onClick={onHide}
                >
                  Cancel
                </button>
