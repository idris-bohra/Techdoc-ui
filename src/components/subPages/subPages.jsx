...
const SubPage = (props) => {
  const { pages, clientData, collections, organizations, pathSlug } =
    useSelector((state) => ({
      pages: state.pages,
  ...
  const renderBody = (subPageId) => {
    ...
    const expanded =
      clientData?.[subPageId]?.isExpanded ?? isUserOnPublishedPage;
    const isSelected =
      isUserOnPublishedPage &&
      isUserOnTechdocOwnDomain &&
      sessionStorage.getItem('currentPublishIdToShow') === subPageId
        ? 'selected'
        : isDashboardRoute && params.pageId === subPageId
          ? 'selected'
          : '';
    const idToRender = sessionStorage.getItem(
    ...
    return (
      <div className='sidebar-accordion accordion ' id='child-accordion'>
        <button tabIndex={-1} className={`p-0 ${expanded ? 'expanded' : ''}`}>
          <div
            className={`active-selected d-flex justify-content-between align-items-center rounded ${isSelected ? ' selected text-dark' : ''} text-secondary `}
          ...
          >
            ...
            >
              ...
              >
                ...
                <IoDocumentTextOutline
                  size={18}
                  className='collection-icons d-inline'
                />
              </span>
              <div
                className={`sidebar-accordion-item d-inline sub-page-header text-truncate fw-500 `}
              >
            ...
            !collections[props.collection_id]?.importedFromMarketPlace ? (
              <div className='sidebar-item-action align-items-center'>
                <div
                  onClick={() => openAddSubPageModal(subPageId)}
                ...
                <div className='dropdown-menu dropdown-menu-right'>
                  <div
                    ...
                    onClick={() => openEditSubPageForm(pages[subPageId])}
                  >
