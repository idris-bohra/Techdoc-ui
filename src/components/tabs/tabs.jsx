...
const CustomTabs = (props) => {
  ...
  const openTabAtIndex = (index) => {
    ...
    if (tabsOrder[index])
      tabService.selectTab(tabsOrder[index], { router, params });
  };
  ...
  const onDrop = (e, droppedOnItem) => {
    ...
    if (
      draggedItem.current === droppedOnItem ||
      !draggedItem.current ||
      !tabsOrder.includes(draggedItem.current)
    ) {
  ...
  const rightHideTabs = () => {
    return Number.parseInt(
      (navRef.current?.scrollWidth - navRef.current?.clientWidth - leftScroll) /
        200
    );
  };
  ...
  const handleOnConfirm = (tabId) => {
    const show_save_prompt_for = showSavePromptFor.filter(
      (tab) => tab != tabId
    );
    ...
    switch (tab.type) {
      case 'history':
        if (historySnapshots[tabId]) {
          if (tab.previewMode) {
            return (
              <div className='d-flex align-items-center '>
                <LuHistory size={16} />
                <span className='mr-2 fw-500 font-12'>
            ...
            return (
              <>
                <div className='d-flex align-items-center '>
                  ...
                  <span className='mr-2 fw-500 font-12'>
                    {historySnapshots[tabId].endpoint.name ||
                      historySnapshots[tabId].endpoint.BASE_URL +
                        historySnapshots[tabId].endpoint.uri ||
                      'Random Trigger'}
                  </span>
                </div>
              </>
          ...
          return (
            <div className=''>{tab.state?.data?.name || 'Random Trigger'}</div>
          );
        }
            ...
            return (
              <div className='d-flex justify-content-center fw-500 font-12 align-items-center'>
                {endpoint.protocolType === 1 && (
                  <div
                    className={`${tabState[tabId]?.draft?.data?.method}-TAB mr-2  request-type-bgcolor`}
                  >
                    {tabState[tabId]?.draft?.data?.method || 'GET'}
                  </div>
                )}
                {endpoint.protocolType === 2 && (
                  <GrGraphQl className='mr-2 graphql-icon' size={14} />
                )}
          ...
          return (
            <div className='d-flex fw-500 font-12 justify-content-end align-items-end'>
              {endpoint?.draft?.protocolType === 1 && (
                <div
                  className={`${endpoint?.draft?.data?.method}-TAB mr-2 request-type-bgcolor`}
                >
                  {endpoint?.draft?.data?.method}
                </div>
              )}
              {endpoint?.draft?.protocolType === 2 && (
                <GrGraphQl className='mr-2 graphql-icon' size={14} />
              )}
          ...
          {tabsOrder.map((tabId) => (
            ...
            >
              <Nav.Item
                ...
                className={
                  tabs?.activeTabId === tabId ? 'active' : 'text-black-50'
                }
                onMouseEnter={() => {
                  setShowPreview(true);
                  setPreviewId(tabId);
              ...
              >
                <Nav.Link
                  className='p-0 d-flex align-items-center bg-white pr-2'
                  eventKey={tabId}
                >
                  <button
                    className='btn truncate bg-white pr-0'
                    onClick={() =>
                      tabService.selectTab(tabId, { router, params })
                    }
                    onDoubleClick={() => {
                  ...
                  >
                    <div
                      className={`d-flex ${tabs?.activeTabId === tabId ? 'tab-name' : ''}`}
                    >
                      {renderTabName(tabId)}
                    </div>
                  </button>
                  <button
                    className='close ml-1 rounded'
                    onClick={() => handleCloseTabs([tabId])}
                  >
                    <i className='uil uil-multiply' />
                  </button>
                  {tabState[tabId]?.isModified ? (
                    <GoDotFill size={12} className='modified-dot-icon' />
                  ) : (
                    ''
                  )}
                </Nav.Link>
        ...
        <div className='d-flex'>
          <Nav.Item className='tab-buttons' id='options-tab-button'>
            {tabs?.tabsOrder?.length > 1 && (
              <OverlayTrigger
                placement='bottom'
                overlay={showTooltips('close-all')}
              >
                <button className='close-all' onClick={handleCloseAllTabs}>
                  {' '}
                  <VscCloseAll />
                </button>
              </OverlayTrigger>
