  ...
  if (tab?.type && tab?.id) {
    switch (tab?.type) {
      ...
      case 'collection':
        ...
        navigatePath = tab?.state?.pageType
          ? pageTypePath[tab?.state?.pageType]
          : navigatePath;
        break;
      case 'manual-runs':
...
function markTabAsModified(tabId) {
  if (getCurrentUser()) {
    ...
    if (!tab.isModified) {
      store.dispatch(
        updateTab(tabId, { previewMode: false, isModified: true })
      );
    }
  }
}
...
function unmarkTabAsModified(tabId) {
  if (getCurrentUser()) {
    ...
    if (tab.isModified) {
      store.dispatch(
        updateTab(tabId, { previewMode: false, isModified: false })
      );
    }
  }
}
