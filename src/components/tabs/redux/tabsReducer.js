...
function tabsReducer(state = initialState, action) {
  ...
  switch (action.type) {
    ...
    case tabsActionTypes.OPEN_IN_NEW_TAB:
      ...
      tabs.tabsOrder = state.tabsOrder.includes(action.tab.id)
        ? [...state.tabsOrder]
        : [...state.tabsOrder, action.tab.id];
      return tabs;
    ...
    case tabsActionTypes.FETCH_TABS_FROM_REDUX:
      tabs = {
        ...
        activeTabId: action.tabsMetadata.activeTabId
          ? action.tabsMetadata.activeTabId
          : state.activeTabId,
      };
    ...
    case tabsActionTypes.REPLACE_TAB_ID:
      ...
      const newOrder = state.tabsOrder.map((item) => {
        if (item === action.payload.currentActiveTabId)
          return action.payload.newTabId;
        else return item;
      });
      tabs = {
        ...state,
    ...
    case tabsActionTypes.UPDATE_PRE_POST_SCRIPT:
      tabs = { ...state };
      tabs.tabs[action.payload.tabId].postScriptExecutedData =
        action.payload?.executedData?.postScriptExecution || '';
      tabs.tabs[action.payload.tabId].preScriptExecutedData =
        action.payload?.executedData?.preScriptExecution || '';
      return tabs;
    ...
    case tabsActionTypes.SET_INTROSPECTION_SCHEMA:
      tabs = { ...state };
      tabs.tabs[action.payload.tabId].introspectionSchemaData =
        action.payload?.schemaData || null;
      return tabs;
