...
import ShowHeaderFooterPreviewModal from './modal/headerFooterPreview';
...
const PublishDocForm = (props) => {
  ...
  const collectionHeader = useSelector(
    (state) => state.collections[collectionId]?.docProperties?.defaultHeader
  );
  const collectionFooter = useSelector(
  ...
  const [footer, setFooter] = useState(collectionFooter || '');
  ...
  const setSelectedCollection = () => {
    ...
    if (collections) {
      ...
      if (collection && Object.keys(collection)?.length > 0) {
        title =
          collection?.docProperties?.defaultTitle ||
          collection?.name ||
          publishDocFormEnum.NULL_STRING;
        logoUrl =
          collection?.docProperties?.defaultLogoUrl ||
          publishDocFormEnum.NULL_STRING;
        domain =
          collection?.customDomain || collection?.path
  ...
  const handleChange = (e, isURLInput = false) => {
    ...
    if (isURLInput) {
      newData[e.currentTarget.name] = handleChangeInUrlField(
        newData[e.currentTarget.name]
      );
    }
    setData(newData);
  };
  ...
  const handleBlur = (e, isURLInput = false) => {
    const newData = { ...data };
    if (isURLInput) {
      newData[e.currentTarget.name] = handleBlurInUrlField(
        newData[e.currentTarget.name]
      );
    }
    setErrors({});
  ...
  const schema = {
    title: Joi.string()
      ...
      .required()
      .trim()
      .label(publishDocFormEnum.LABELS.title),
    domain: Joi.string()
      .allow('')
    ...
    logoUrl: Joi.string()
      .trim()
      .allow('')
      .label(publishDocFormEnum.LABELS.logoUrl),
  ...
  const saveAndPublishCollection = (selectedCollection) => {
    ...
    if (fileSize > 50 && fileSize != NaN) {
      newErrors = {
        ...newErrors,
        icon: "Image size shouldn't be greater than 50KB",
      };
    }
    setErrors(newErrors || {});
    if (newErrors) return;
  ...
  const getDisabledStyle = (disabled) =>
    disabled ? { cursor: 'not-allowed', opacity: 0.4 } : { cursor: 'pointer' };
          ...
          <div className='uploadBox' style={getDisabledStyle(data.logoUrl)}>
            {!binaryFile && (
              <div className='d-flex align-items-center'>
                {renderUploadModule(data.logoUrl)}
              </div>
            )}
            {binaryFile && (
              <img
                src={`data:image/png;base64,${binaryFile}`}
                height='60'
                width='60'
                alt='data'
              />
  ...
  const renderInput = (name, disabled, placeholder, isURLInput = false) => {
    ...
    return (
      <div className='form-group mb-4'>
        ...
        />
        {name === 'domain' && (
          <span className='domain-info font-12 mt-1 d-block text-success'>
            {`Point c name of the above domain to ${MAPPING_DOMAIN}`}
            <a
              className='ml-1 text-success'
              href='https://techdoc.walkover.in/p/White-Labelling?collectionId=2Uv_sfKTLPI3'
            >
              Learn More
            </a>
          </span>
        ...
        {name === 'title' && (
          <span className='domain-info font-12 mt-1 d-block'>
            Collection name will be used by default when no title is entered.
          </span>
        )}
  ...
  const isCollectionPublished = (selectedCollection) =>
    selectedCollection?.isPublic || false;
  ...
  const renderPublicUrl = () => {
    ...
    const url =
      process.env.NEXT_PUBLIC_UI_URL + '/p?collectionId=' + collectionId;
    const isDisabled = IsParentPagePublishedInACollection(
      collections[collectionId]?.rootParentId
    );
    ...
    return (
      <div>
        <div className='published-post d-flex align-items-center mt-4 mb-1'>
          ...
          <div className='api-label POST request-type-bgcolor ml-2 w-auto px-1 '>
            {' '}
            published{' '}
          </div>
        </div>
        <OverlayTrigger
          overlay={
            <Tooltip
              ref={tooltipRef}
              id='tooltip-unpublished-endpoint'
        ...
        >
          ...
          >
            ...
            <span
              ...
              className={
                isDisabled ? 'text-disable flex-grow-1' : 'disabled-link'
              }
            >
              {url}
            </span>
  ...
  const openPublishSidebars = () => (
    <>
      {isPublishSliderOpen && (
        <PublishSidebar {...props} closePublishSidebar={closePublishSidebar} />
      )}
    </>
  );
  ...
  const renderActionButtons = (publishCheck) => {
    ...
    const rootParentId =
      collections[props.selected_collection_id]?.rootParentId;
    ...
    return (
      <div className='mt-2'>
        ...
        <Button
          className={
            loader ? 'buttonLoader m-1 btn-sm font-12' : 'm-1 btn-sm font-12'
          }
  ...
  const handleSave = async () => {
    ...
    const response = await collectionsApiService.updateCollection(
      props.selected_collection_id,
      {
      ...
      <div className='d-flex justify-content-center'>
        <div className={'publish-on-tab'}>
          ...
          <span className='mt-2 d-inline-block'>
            Completing this step will make your collection available at a public
            URL.
          </span>
              ...
              metaType !== 0 && (
                <div className='form-group mb-4'>
                  ...
                  {publicEnv === null ||
                    ...
                    <Dropdown>
                      <Dropdown.Toggle
                        className='justify-content-between bg-white border w-100 font-12'
                        variant='light'
                        id='dropdown-basic'
                      >
                        {'Select Environment'}
                    ...
                    <input
                      type='text border p-1 rounded cursor-pointer border rounded'
          ...
          {showCreateEnvForm && (
            <Modal
              className='main-modal-contanier'
              show={showCreateEnvForm}
              onHide={() => setShowCreateEnvForm(false)}
            >
                      ...
                      <tr>
                        <th className='text-center'>
                          <Dropdown>
                            <IconButton>
                              <Dropdown.Toggle
                                className='select-check p-0 bg-transparent text-dark border-0'
                                id='dropdown-basic'
                              >
                                Select
                              </Dropdown.Toggle>
              ...
              <Modal.Footer className='justify-content-between'>
                <Button
                  className='add-more-button text-grey bg-white font-12 border-0'
          ...
          <div>
            <Button
              ...
              id='publish_collection_btn'
              variant='btn btn-outline'
              className='m-1 btn-sm font-12 mb-1'
            >
              {showEditor
                ? 'Close Header/Footer'
            ...
            {showEditor && (
              <ShowHeaderFooterPreviewModal
                tabs={['Header', 'Footer']}
                header={header}
                footer={footer}
                show={showEditor}
                onHide={() => setShowEditor(false)}
                setHeader={(event) => setHeader(event)}
                setFooter={(event) => setFooter(event)}
                saveAndPublishCollection={() =>
                  saveAndPublishCollection(getSelectedCollection())
                }
              />
            )}
          </div>
