...
import emptyImage from '../../assets/web/empty.png';
...
const ContentPanel = () => {
    ...
    if (pathname === `/orgs/${params.orgId}/dashboard`) {
      ...
      if (tabs?.tabsOrder?.length) {
        ...
        if (tab?.type === 'manual-runs') {
          router.push(
            `/orgs/${orgId}/dashboard/collection/${tab?.state?.collectionId}/runs/${tab?.id}`
          );
          return;
        }
  ...
  const renderLandingDashboard = () => {
    ...
    if (orgMeta === null || orgMetaType === 1) {
      return (
        <>
          <div className='no-collection h-100vh d-flex flex-d-col justify-content-center align-items-center flex-wrap'>
      ...
      return (
        <>
          <div className='no-collection h-100 d-flex flex-d-col justify-content-center align-items-center flex-wrap'>
            ...
            <p className='m-0 text-grey techdoc-journey'>
              Begin Your Journey to Excellence in Documentation
            </p>
          </div>
        </>
      );
  ...
  return (
    <main role='main' className='main h-100vh'>
      <Tab.Container
        ...
        defaultActiveKey={tabs.activeTabId}
        activeKey={tabs.activeTabId}
      >
        {getCurrentUser() ? (
          <>
            <div className='content-header border-bottom align-items-center'>
              <div
                className='mx-3 my-2 py-1 px-2 rounded cursor-pointer sidebar-open-icon'
