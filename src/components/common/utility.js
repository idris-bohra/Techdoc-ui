...
export const msgText = {
  ...
  unpublishEndpoint:
    'You are about to Unpublish Endpoint from your Public API doc.',
...
export function isDashboardRoute(props, sidebar = false) {
  if (
    ...
    (props.location.pathname.includes('/dashboard') ||
      ...
      (sidebar === true &&
        props.location.pathname.includes('/orgs/:orgId/admin/publish')) ||
      (sidebar === true &&
        props.location.pathname.includes('/orgs/:orgId/admin/feedback')))
  ) {
    return true;
  } else return false;
}
...
export function isValidDomain() {
  const domainsList = process.env.NEXT_PUBLIC_DOMAINS_LIST
    ? process.env.NEXT_PUBLIC_DOMAINS_LIST.split(',')
    : [];
...
export function isDashboardAndTestingView(props, view) {
  return (
    isDashboardRoute(props) && (view === 'testing' || !isSavedEndpoint(props))
  );
}
...
export function sensitiveInfoFound(endpoint) {
  ...
  if (typeof endpoint?.params?.access_token === 'object') {
    const value =
      typeof endpoint.params.access_token.value === 'string'
        ? endpoint.params.access_token.value
        : '';
  ...
  if (typeof endpoint?.headers?.Authorization === 'object') {
    const value =
      typeof endpoint.headers.Authorization.value === 'string'
        ? endpoint.headers.Authorization.value
        : '';
    const authData = value.split(' ');
    if (authData?.length === 1) {
      try {
...
const modifyEndpointContent = (endpointData, untitledData) => {
  ...
  untitled.authorizationData =
    endpoint?.authorizationData || untitled.authorizationData;
...
export function getOnlyUrlPathById(id, sidebar, mode = 'external') {
  ...
  while (sidebar?.[id]?.type > 0) {
    const itemName =
      mode === 'internal' ? sidebar[id].name : sidebar[id].urlName;
    path.push(itemName);
...
const deleteSidebarData = (pages, tabs, pageId, deletedTabIds, deletedIds) => {
  if (pages[pageId]) {
    ...
    pages[pageId]?.type == 4
      ? deletedIds.endpointIds.push(pageId)
      : deletedIds.pageIds.push(pageId);
...
export const deleteAllPagesAndTabsAndReactQueryData = async (
  pageId,
  collectionId
) => {
  try {
    ...
    if (parentId != null) {
      pages[parentId].child = pages[parentId].child.filter(
        (id) => id !== pageId
      );
    }
...
export const modifyDataForBulkPublish = (
  collectionData,
  allPagesData,
  collectionId
) => {
  ...
  const formatedData = {
    ...
    children: modifiedData(
      allPagesData?.[rootParentId]?.child || [],
      allPagesData
    ),
  };
  return formatedData;
...
export const modifyCheckBoxDataToSend = (
  flattenData,
  allSelectedIds,
  dataToPublishSet
) => {
...
export const parseCronExpression = (cronExpression) => {
  const [minute, hour, , , dayOfWeek] = cronExpression
    .split(' ')
    .map((num) => (num === '*' ? '*' : parseInt(num, 10)));
