...
const DisplayUserAndModifiedData = () => {
  let currentIdToShow = sessionStorage.getItem(
  ...
  const lastModified = pages?.[currentIdToShow]?.updatedAt
    ? moment(pages[currentIdToShow].updatedAt).fromNow()
    : null;
  const user = usersList?.find((user) => user.id === updatedById);

  if (isOnPublishedPage())
    return lastModified && <span>Modified {lastModified}</span>;
