...
function fixSpanTags(html) {
  ...
  if (html.startsWith('</span>')) {
    ...
    html.startsWith("<span text-block='true'>") ||
    html.startsWith('<span text-block="true">')
  ) {
    html = html?.slice(24);
  } else if (
    (!html.startsWith(`<span text-block='true'>`) ||
      !html.startsWith(`<span text-block="true">`)) &&
    (!html.startsWith(`<span variable-block='true'>`) ||
      !html.startsWith(`<span variable-block="true">`))
  ) {
  ...
  if (
    html.endsWith("<span text-block='true'>") ||
    html.endsWith('<span text-block="true">')
  ) {
    html = html?.slice(0, -24);
  } else if (
    html.endsWith("<span variable-block='true'>") ||
    html.endsWith('<span variable-block="true">')
  ) {
...
function replaceParamsHtmlInHostContainerHtml(
  pureHTML,
  updateParams,
  startIndex,
  endIndex,
  ...
  if (
    ...
    newHTML.endsWith("<span variable-block='true'>") ||
    newHTML.endsWith('<span variable-block="true">')
  ) {
...
export {
  ...
  getPathVariableHTML,
  getQueryParamsHTML,
  replaceParamsHtmlInHostContainerHtml,
  getInnerText,
  getIntoTextBlock,
  getIntoVariableBlock,
  convertToHTML,
  decodeHtmlEntities,
  convertTextToHTML,
};
