...
import {
  ...
  restoreCollection,
} from '@/components/collections/collectionsApiService';
...
const TrashPage = () => {
  ...
  const handleRestore = async (collectionId, collectionName) => {
    try {
      ...
      if (error?.response?.data === 'Collection Name Must be Unique')
        setEditableRow({ id: collectionId, name: collectionName });
  ...
  return (
    <Container>
      <div
        className='back-to-workspace mb-4 d-flex align-items-center'
        onClick={handleBack}
                  ...
                  <td className='restore-action'>
                    {editableRow.id === collection.id ? (
                      ...
                      <MdSettingsBackupRestore
                        ...
                        onClick={() =>
                          handleRestore(collection.id, collection.name)
                        }
                      />
                    )}
                  </td>
