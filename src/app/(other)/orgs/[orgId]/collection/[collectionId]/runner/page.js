...
import {
  ...
  updateAllEndpointCheckStatus,
} from '@/store/clientData/clientDataActions';
...
import {
  runAutomations,
  generateDescription,
} from '@/components/collections/runAutomation/redux/runAutomationActions';
...
export default function RunAutomation({ params }) {
  const userEmail =
    JSON.parse(localStorage.getItem('profile'))?.email || 'email not found';
  ...
  const {
    ...
    return {
      ...
      collectionName:
        state.collections?.[params?.collectionId]?.name || 'collection',
  ...
  const [tokenGenerationInProgress, setTokenGenerationInProgress] =
    useState(false);
  ...
  useEffect(() => {
    ...
    const hasIssues = Object.keys(allPages).some(
      (pageId) =>
  ...
  const filterEndpointsOfCollection = () => {
    const endpointsIds = Object.keys(allPages).reduce((acc, pageId) => {
      if (
        allPages[pageId]?.collectionId === params?.collectionId &&
        allPages[pageId].protocolType === 1
      )
        acc.push(pageId);
      return acc;
    }, []);
  ...
  const renderEndpointName = (endpointId) => {
    const hasIssues =
      ...
      !allPages?.[endpointId]?.sampleResponse;
    return (
      <div className='d-flex justify-content-center align-items-center'>
        <span
          className={`api-label ${allPages?.[endpointId]?.requestType} request-type-bgcolor mr-2`}
        >
  ...
  const filterSelectedEndpointIds = () => {
    return endpointsIds.reduce((acc, endpointId) => {
      if (clientData?.[endpointId]?.automationCheck === true)
        acc[endpointId] = allPages?.[endpointId]?.name;
      return acc;
    }, {});
  };
  ...
  const filterSelectedEndpointIdsArray = () => {
    return endpointsIds.filter(
      (endpointId) => clientData?.[endpointId]?.automationCheck === true
    );
  };

  const handleRunAutomation = async () => {
    const filteredEndpointIds = filterSelectedEndpointIds();
    if (Object.keys(filteredEndpointIds)?.length === 0)
      return toast.warn('Please select endpoints');
    if (filteredEndpointIds?.length === 0)
      return toast.error(
        'Please select at least one endpoint to run the automation'
      );
  ...
  const handleAskAi = async () => {
    const endpointIdsWithoutDescriptionAndSampleResponse =
      getEndpointsWithoutDescriptionOrResponse();
    try {
      await dispatch(
        generateDescription(endpointIdsWithoutDescriptionAndSampleResponse)
      );
  ...
  return (
    <div className='run-automation-container'>
      <div className='endpoints-container pt-3'>
        <div>
          <button
            className='btn position-absolute back-button-api-automation btn-sm rounded-circle icon-button'
            onClick={handleBack}
          >
            <FaArrowLeft />
          </button>
          ...
          <span className='small-text'>
            If descriptions and sample responses are not provided, AI will not
            generate the order, and automation will fail.
          </span>
          {/* {showAiIcon && (
          ...
          <h3 className='text-left'>
            Run Automation for {`${collectionName}`}
          </h3>
        </div>
          ...
          <div className='p-3 d-flex flex-column'>
            <div className='d-flex align-items-center justify-content-between'>
              <div className='checkbox-container d-flex align-items-center'>
                <span
                  onClick={() => handleSelectAndDeselectAll(true)}
                  className='ml-1 select-all mr-1 cursor-pointer'
                >
                  Select All
                </span>
                <div className='separation'></div>
                <span
                  onClick={() => handleSelectAndDeselectAll(false)}
      ...
      <div className='options-container'>
        ...
        {runType === 'schedule' && (
          <div>
            ...
            <span>
              Your collection will be automatically run on the Techdoc at the
            ...
            <Form.Group>
              ...
              <Form.Control
                ...
                onChange={(e) => setCurrentEnvironmentId(e.target.value)}
              >
              ...
              <div>
                <Form.Group>
                  ...
                  <span style={{ color: '#ff8000' }}>
                    Make sure to copy your personal access token now as you will
                    not be able to see this again.
                  </span>
        ...
        {runType === 'manual' ? (
          ...
          <div className='d-flex justify-content-between'>
            ...
            <a
              href='https://techdoc.walkover.in/p/webhook?collectionId=lkm5WVs2Hc-2'
              target='_blank'
