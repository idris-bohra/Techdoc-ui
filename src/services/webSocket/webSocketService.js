...
export function initConn(channel) {
  ...
  CLIENT = new WebSocketClient(
    process.env.NEXT_PUBLIC_RTLAYER_OID,
    process.env.NEXT_PUBLIC_RTLAYER_SID
  );
  if (CHANNEL) {
...
const handleMessage = (message) => {
  ...
  if (
    message?.uniqueTabId ==
    sessionStorage.getItem(SESSION_STORAGE_KEY.UNIQUE_TAB_ID)
  )
    return;
  ...
  switch (message.operation) {
    ...
    case OperationTypes.ENDPOINT_DELETE:
      deleteAllPagesAndTabsAndReactQueryData(message.data.endpoint.id).then(
        (data) => {
          handleDeleteActions(data);
        }
      );
